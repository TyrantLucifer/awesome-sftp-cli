...
> [!WARNING]
> `v0.1.14` 是当前公开预览版本线。macOS 产物未经签名或公证，它不是公开的 AMSFTP 1.0；标准 Level 0 SFTP 路径可用，production Helper 和 Level 2 跨主机直传仍为 **CLOSED**。
...
公开预览发布页：[`v0.1.14`](https://github.com/TyrantLucifer/awesome-sftp-cli/releases/tag/v0.1.14)。历史 owner-only 内部预览仍可通过 Git 历史与旧 tag 追溯。
...
该命令先检查渠道和目标版本；只有确有新版本时才通过 owner-only RPC 请求升级停机。Homebrew 安装委托给 `brew upgrade`，独立安装校验发布的 `install.sh` checksum 后复用安装器。升级前 daemon 若在运行，升级后才会恢复；若有正在执行的 Job，命令以 conflict 退出，不会静默中断任务。上面的 `curl` 与 `brew upgrade` 仍可用于恢复或手工维护。
