...
Stage 0 行已按完成证据标记为 `Verified`；Stage 1 的 M1.1–M1.3 已验证，M1.4 实现与真实环境恢复证据正在收口，远端目录 source streaming 仍受冻结依赖 API 阻塞。其余 Stage 2–6 行仍按阶段保持 `Planned` 或在跨阶段验收仅完成 Stage 1 slice 时保持 `In Progress`。实现后必须用可复现的实际证据替换预期证据，包括测试名或命令、验证平台、测试夹具/环境和结果记录；只有代码、测试、阶段验证记录、功能矩阵与 `PROJECT_STATE.md` 一致时，状态才能改为 `Verified`。
...
| CONN-003 | CLI 双路径启动 | 1 | Implemented | `amsftp <left> <right>` 接受本地路径与 `host:path`，两个初始位置均被解析、连接并独立报告错误。 | Zero/one/two Location parser, stable Endpoint startup and pane-local error tests pass; the exact Hosted three-combination recovery candidate is still being finalized. See [Stage 1 verification](../verification/stage-01.md). |
...
| CONN-006 | 自动断线检测与重连 | 1 | Implemented | EOF、网络中断和 server restart 被识别为连接状态变化；只读操作可安全重发，写任务交给恢复状态机。 | Typed transport interruption, bounded reconnect policy, stale-generation rejection and nearest-parent recovery tests pass; exact sshd-restart PTY evidence is pending the active Hosted candidate. See [Stage 1 verification](../verification/stage-01.md). |
| CONN-007 | 会话级能力探测 | 1 | Implemented | 建连后记录 SFTP 扩展、路径/rename 能力和 helper 可用性；重连后重新探测，旧能力不盲目复用。 | Capability snapshots carry session/generation revisions; fake/provider and pane transaction tests prove replacement, withdrawal and stale-revision rejection. A changed real-server capability fixture is not yet recorded. See [Stage 1 verification](../verification/stage-01.md). |
...
| CONN-009 | 连接错误带上下文 | 1 | In Progress | 错误至少包含端点显示名、阶段、OpenSSH 退出/标准错误摘要与可行动建议，不只显示“连接失败”。 | Safe client summaries include request ID, code, endpoint and retry/effect while daemon logs preserve bounded cause; auth, host-key, transport and subsystem classifications are tested. A complete DNS/refused golden matrix remains open. See [Stage 1 verification](../verification/stage-01.md). |
...
| PANE-002 | 任意端点组合 | 1 | Implemented | local/local、local/remote、remote/local、同 remote 双路径和 remote A/remote B 均能浏览。 | Independent per-pane local/SSH startup and two real sshd sessions are implemented; the exact Hosted PTY combination matrix is pending the active recovery candidate. See [Stage 1 verification](../verification/stage-01.md). |
| PANE-003 | 栏内独立切换端点 | 1 | Implemented | 用户可只改变活动栏的端点，另一栏路径、选择和连接保持不变。 | Endpoint modal/reducer tests prove only the active pane changes and failed switches preserve both committed panes; final Hosted recovery evidence is pending. See [Stage 1 verification](../verification/stage-01.md). |
| PANE-004 | 增量目录加载 | 1 | In Progress | 目录项以流式/分页方式进入模型，首屏无需等待完整目录读取，加载可取消。 | 256-entry daemon pages, generation-safe reducer, slow-Fake responsiveness, cancellation and visible-window rendering pass. Exact pkg/sftp v1.13.10 still buffers the full remote directory before its public `ReadDirContext` returns, so true remote source streaming is explicitly blocked pending an approved dependency decision. See [Stage 1 verification](../verification/stage-01.md). |
...
| ID | 能力 | Stage | 状态 | 验收标准 | 当前证据 |
|---|---|---:|---|---|---|
| WORK-001 | 保存双栏工作区 | 1 | Implemented | 工作区保存两栏端点引用、路径、排序/过滤、布局与缓存策略，保存动作原子化。 | Versioned document/store round-trip, owner-private `0600`, interrupted replacement, corrupt preservation and daemon route tests pass; final real PTY reopen is pending. See [Stage 1 verification](../verification/stage-01.md). |
| WORK-002 | `--workspace` 启动 | 1 | Implemented | `amsftp --workspace <name>` 恢复工作区；缺失端点或路径逐栏报错且允许修复。 | CLI exclusivity, stable Endpoint reference restoration, corrupt/missing fallback picker and pane-local recovery tests pass; final Hosted reopen is pending. See [Stage 1 verification](../verification/stage-01.md). |
...
| DAEM-002 | 权限受限 Unix socket 与路径信任链 | 1 | Implemented | config/state/cache/log/runtime应用根为euid-owned真实`0700`，文件/socket不宽于`0600`；ancestor/ssh用integrity-only，owner roots/files用owner-private。仅受测APFS/ext4/XFS（runtime可含受测tmpfs）明确no-ACL/unsupported可回到DAC；真实ACL查询/解析错误、NFSv4/CIFS/未知FS、不安全path/override均fail closed。runtime唯一宽松DAC回退是root-owned sticky`01777 /tmp`；peer非当前uid不可连接且无TCP。 | Path/ACL/lock/socket tests now include Darwin kernel deny/direct/inherited allow profiles, Linux kernel access/default ACLs, cross-process lock exclusivity and a root-gated real hostile-other-UID socket peer. Local platform race/oldstable pass; the new four-platform Hosted step is pending. See [Stage 1 verification](../verification/stage-01.md). |
...
| ID | 能力 | Stage | 状态 | 验收标准 | 当前证据 |
|---|---|---:|---|---|---|
| SEC-001 | 本地 IPC 最小暴露 | 1 | Implemented | 只创建当前用户 `0600` Unix socket，不监听 TCP；请求关联到已验证的本地用户上下文。 | Unix-only `0600` socket, lock/stale-socket and peer checks pass; a root-gated fixture deliberately widens DAC and proves a real other-UID peer is still rejected. Local compilation/race passes and four-platform Hosted execution is pending. See [Stage 1 verification](../verification/stage-01.md). |
