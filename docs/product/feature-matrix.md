...
| PANE-009 | 符号链接可见且递归保守 | 2 | In Progress | 链接目标信息可见；递归复制/搜索默认不跟随目录链接，除非用户策略显式允许并有循环保护。 | M2.3 discovery emits symlinks as visible items but never recurses into or copies them; depth and descendant boundaries are independently enforced. Exact-candidate Hosted evidence is pending. |
...
| JOB-004 | 不可变操作计划 | 2 | In Progress | 执行开始后源集合、目标、冲突/完整性策略和初始路由被冻结；变更要求新建或显式修订计划并留痕。 | `internal/transfer` 已冻结 FileRef、源/目标能力 revision、route、SHA-256 验证、冲突策略、part 与 buffer budget；调用方随后修改 clipboard/Location/指针字段不改变 Plan 或持久 JSON。daemon/TUI 接线仍在 M2.2。 |
...
| XFER-002 | 本地与远端任意方向复制 | 2 | In Progress | local→local、local→remote、remote→local、同 remote 和 remote A→remote B 都通过同一 Job 模型完成。 | Single-file PTY covers local and both SFTP directions; M2.3 real fixtures cover same-remote and independent remote A→B directory copies through the same frozen Plan/Worker. Hosted promotion is pending. |
| XFER-003 | remote A→B 默认本机中继 | 2 | In Progress | 不满足服务端路径时，daemon 使用固定大小缓冲从 A 读向 B 写，默认不把完整文件落本地磁盘。 | Two independent temporary sshd sessions pass remote A→B directory relay with a 7-byte stream budget and no content cache/local full-file path; Hosted promotion is pending. |
| XFER-004 | 有界内存与背压 | 2 | In Progress | 读取速度受目标写入背压约束；缓冲总量与并发配置成比例，不随文件大小增长。 | Plans freeze a 256 KiB stream buffer plus 64-item/256-page directory budgets; a 100 GiB synthetic source checkpoints at 64 KiB and the million-entry producer stays within a 17-item test queue. Resume has a fixed two-buffer 512 KiB ceiling. |
| XFER-005 | 目标临时 part | 2 | In Progress | 新内容先写目标目录中的唯一 `.part-<job-id>`；未验证完成前最终目标路径不出现新内容。 | Planner 冻结同目录唯一 `.<final>.part-<job-id>`，worker 在 verified checkpoint 回调中证明 final 不存在；完整 kill-9 矩阵仍待补齐。 |
| XFER-006 | 验证后原子提交 | 2 | In Progress | 完成大小与策略要求的完整性验证后，使用端点支持的安全 rename 提交；无法保证原子时计划明确标示并采用保守流程。 | Worker 先重读 part 做 SHA-256，再重查 final 并提交；LocalFS/SFTP no-replace 使用 hard-link 原子出现并分离 part cleanup，replace 只在冻结覆盖策略和目标 fingerprint 匹配时执行。真实 sshd Hosted 证据待运行。 |
| XFER-007 | 断点与检查点 | 2 | In Progress | 可恢复端点记录已确认偏移、源版本和 part 身份；恢复前验证三者仍匹配，不匹配则重启或等待决策。 | V1 `job_checkpoints` guarded upsert/get 与 JobJournal 持久化 offset、source/part fingerprint、SHA-256 state；关闭并重开 SQLite 后新 worker 从安全 offset 完成。断网/kill-9 矩阵仍待补齐。 |
| XFER-008 | 完整性策略 | 2 | In Progress | 基线至少验证大小和传输边界；有 hash 能力时可选择强哈希；策略和实际验证证据记录到 Job。 | Frozen Plan 使用 `stream_sha256`；worker 比对源大小、重读 part hash、提交后再读 final hash并持久记录实际 digest。多策略/弱验证 UI 留待后续。 |
| XFER-009 | 目录递归复制 | 2 | In Progress | 递归遍历流式产生子任务，保留相对结构；权限错误和部分失败按条目记录，不将整树预载入内存。 | Bounded discovery streams one million synthetic entries, nested relative structure is preserved, and daemon restart revalidates the owned root/children before continuing. Per-item partial-result closeout remains open. |
| XFER-010 | 目录创建与元数据策略 | 2 | In Progress | 目标目录按需创建；权限/时间等元数据只按显式支持与策略保留，不宣称无法保证的所有权语义。 | LocalFS and real SFTP directory workers create the frozen root and descendants on demand; no unsupported ownership/time preservation is claimed. Hosted promotion is pending. |
| XFER-011 | 短读短写安全处理 | 2 | In Progress | read/write 返回部分字节时正确推进或失败，不跳过数据、不把短写当成功。 | Worker 的 bounded read loop 与 write-all loop 拒绝越界/零进展并仅按实际字节推进；Fake mutation contract 已覆盖短写 effect。全偏移属性与断网组合仍待 M2.2 故障矩阵。 |
...
| XFER-015 | 失败条目可重试 | 2 | In Progress | 批量任务可只重试失败/跳过条目，已验证成功项不会重复覆盖；重试继承或重新确认相关策略。 | A permission-denied item enters durable retry_wait while its sibling remains committed; after repair, resume content-revalidates the sibling and copies only the missing item. Hosted promotion is pending. |
| XFER-016 | 传输结果清单 | 2 | In Progress | 任务完成时记录每项源、目标、字节、提交方式、验证级别和异常，不仅提供汇总百分比。 | Directory results persist a bounded 256-item source/destination/status/bytes/error-code manifest, complete aggregate counts and an explicit truncation count; mixed success/failure tests pass locally. |
...
| ID | 能力 | Stage | 状态 | 验收标准 | 当前证据 |
|---|---|---:|---|---|---|
| CONF-001 | 目标存在冲突检测 | 2 | In Progress | 计划和提交前均检测目标存在/变化；陈旧预检不能授权静默覆盖。 | Planner 检测初始 final，worker 在 verified part 后重检；测试在两者之间创建 racing final 并证明 ask/skip/overwrite/auto-rename 都不会静默采用陈旧结果。 |
| CONF-002 | 明确冲突策略 | 2 | In Progress | 支持跳过、覆盖、重命名/另存为和取消；目录合并与文件覆盖分别呈现。 | 单文件 worker 已实现 ask、skip、overwrite、auto-rename 和 cancel/pause part 保留；Job-local apply-all、目录和 UI 决策留痕仍待 M2.2/M2.3。 |
...
| CONF-005 | 覆盖仍使用临时提交 | 2 | In Progress | 覆盖策略不直接截断最终目标；新内容验证成功后再以端点安全流程替换。 | 覆盖竞态测试证明旧 final 保持到 part 完整 SHA-256 验证，随后以 expected-destination fingerprint replacement 提交；故障矩阵仍待扩展。 |
...
| SCALE-004 | 有界工作队列 | 2 | In Progress | 递归生产者受消费者和持久队列背压；队列容量可配置并有安全默认，不因深树耗尽内存。 | Discovery freezes 64 queue items, 256 entries/page and depth 128 by default; the million-entry fixture proves capacity 17/page 31 remain hard ceilings. Exact-candidate Hosted evidence is pending. |
