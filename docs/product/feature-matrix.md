...
| ID | 能力 | Stage | 状态 | 验收标准 | 当前证据 |
|---|---|---:|---|---|---|
| OBS-001 | Preview/Jobs/Log 底部抽屉 | 3 | In Progress | 三个页签可独立打开/切换/关闭，保留双栏上下文；异常或等待任务有非侵入提示。 | Bounded bottom-region layout, normal/narrow rendering, Jobs reuse, tab/focus transitions and pane-context preservation pass; Log currently exposes only an empty bounded state, so snapshot/replay/filter/redaction and workspace restore remain open. See [Stage 3 verification](../verification/stage-03.md#m31--drawer-and-cache-foundation). |
