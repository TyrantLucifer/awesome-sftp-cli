...
| ID | 能力 | Stage | 状态 | 验收标准 | 当前证据 |
|---|---|---:|---|---|---|
| AUTH-001 | 系统 OpenSSH 为唯一 SSH 实现 | 1 | In Progress | 远端会话按 ADR-0001 以 validated absolute OpenSSH path 和精确安全 argv 启动 SFTP subsystem；默认 `/usr/bin/ssh`、不查找 PATH，产品不实现第二套 SSH/Kerberos 协议栈。 | Exact argv/host/path tests and real-sshd Hosted fixture are implemented; Hosted result and full negative config matrix pending. See [Stage 1 verification](../verification/stage-01.md). |
