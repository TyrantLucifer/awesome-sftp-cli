...
> - 基线：已批准产品设计
> - 实现状态：Stage 0–2 已完成；Stage 3–6 尚未开始
> - 最后更新：2026-07-16
...
| VIM-016 | `K`/`J`/`L` 抽屉快捷键 | 3 | Implemented | 大写 `K` 打开 Preview、`J` 打开 Jobs、`L` 打开 Log；重复按键按定义收起或聚焦，不与小写导航串线。 | Pure reducer and tcell tests cover uppercase K/J/L open/switch/refocus/close, Esc-to-pane, lowercase navigation separation and pane-context preservation; normal/narrow drawer snapshots, 20-repeat focused runs, targeted race and full `make check` pass. See [Stage 3 verification](../verification/stage-03.md#m31--drawer-and-cache-foundation). |
...
| PREV-002 | 代码与 JSON 结构化显示 | 3 | In Progress | 常见代码按语法高亮，合法 JSON 可格式化且保留原始查看；解析失败回退为纯文本并说明。 | Complete JSON within 256 KiB/depth 64 is pretty-printed under 512 KiB/10,000-line output budgets; invalid, partial and over-depth JSON falls back to terminal-safe text with an explicit warning. Syntax color and raw/formatted toggle remain open. See [Stage 3 verification](../verification/stage-03.md#m32--built-in-image-and-external-preview). |
...
| PREV-009 | 二进制安全识别 | 3 | Implemented | 检测到二进制或解码错误时不向终端倾倒控制字节，改为十六进制摘要/元数据或外部预览。 | NUL/binary input is rendered as a bounded numbered hex dump; control and invalid UTF-8 bytes in text are escaped before TUI rendering, and image payloads produce metadata rather than raw terminal bytes. Focused and race tests pass. See [Stage 3 verification](../verification/stage-03.md#m32--built-in-image-and-external-preview). |
| PREV-010 | 预览请求可取消且防串片 | 3 | In Progress | 快速移动光标时旧请求被取消或结果按请求 ID 丢弃，不会把旧文件内容显示到新选择上。 | Drawer-visible cursor and pane changes now emit cancel before the new bounded Preview intent, while the inherited generation reducer still discards stale chunks. Daemon-owned 128-bit request ID, Endpoint-generation and full high-frequency/out-of-order evidence remain open. See [Stage 3 verification](../verification/stage-03.md#m31--drawer-and-cache-foundation). |
...
| EDIT-002 | Vim-first 编辑器解析 | 3 | Implemented | 顺序为显式`executable+argv[]`、`VISUAL`、`EDITOR`、`nvim`、`vim`、`vi`；env只做无expansion/operator的受限词法切分，PATH仅发现，启动前解析/显示absolute executable并direct exec，文件为独立absolute末参数。 | Structured explicit command then `VISUAL`/`EDITOR` and nvim/vim/vi precedence, quote/backslash grouping for env values, operator/substitution/glob/control rejection, PATH→canonical absolute freezing, executable/materialization identity revalidation, scrubbed env and final separate file arg pass repeated/race/vet tests. TUI launch is tracked by EDIT-001. See [Stage 3 verification](../verification/stage-03.md#m33--editor-and-default-opener). |
...
| EDIT-008 | `o` 使用本机默认应用 | 3 | In Progress | macOS固定`/usr/bin/open`、Ubuntu固定`/usr/bin/xdg-open`，或结构化显式opener；direct exec且canonical absolute文件路径为独立arg。远端文件先入租约缓存，下游同UID app是用户/OS边界。 | Exact platform defaults and structured explicit opener resolution now freeze/revalidate canonical executable identity and use a final canonical file argument without a shell. Cache lease and GUI lifecycle integration remain open. See [Stage 3 verification](../verification/stage-03.md#m33--editor-and-default-opener). |
...
| ID | 能力 | Stage | 状态 | 验收标准 | 当前证据 |
|---|---|---:|---|---|---|
| CACHE-001 | 默认短期 LRU | 3 | Implemented | 内容缓存有全局和工作区配额，超过配额时按 LRU 回收未固定且无租约对象。 | Manual-clock tests prove deduplicated global/workspace accounting, deterministic bounded LRU and all-protected diagnostics while pinned, dirty, deleting, leased, shared, referenced and materialization-reachable objects remain protected. Filesystem deletion execution remains gated. See [Stage 3 verification](../verification/stage-03.md#m31--drawer-and-cache-foundation). |
| CACHE-002 | 内容寻址与去重 | 3 | In Progress | 相同内容只保存一份 blob，不同 Location 通过元数据引用；引用计数/可达性在崩溃后可重建。 | Typed SHA-256 Blob IDs, golden Location+fingerprint Entry IDs, independent entries, shared accounting and reachability protection are green. Atomic filesystem publication, durable catalog binding and restart rebuild are still in progress. See [Stage 3 verification](../verification/stage-03.md#m31--drawer-and-cache-foundation). |
...
| CACHE-005 | 缓存租约 | 3 | In Progress | preview/edit/open/上传中的 blob 有可续租的 owner；过期 owner 可回收，活跃 owner 不被清理。 | Typed leases implement manual-clock heartbeat/release, expiry+grace, PID+birth match/mismatch and fail-closed uncertain classification; LRU consumes that classification. Durable restart loading and kill-9 integration remain open. See [Stage 3 verification](../verification/stage-03.md#m31--drawer-and-cache-foundation). |
...
| SEC-008 | 外部进程与remote command边界 | 3 | In Progress | 只有ssh使用严格owner/ACL validated-absolute resolver。editor/opener是用户授权同UID边界，但须结构化配置、PATH发现后absolute direct exec、文件独立arg。Helper是唯一app-generated restricted string；显式`!`/`gs`用ADR-0001 fresh argv/marker/Q/TTY契约，Provider/RPC不可隐式触发。 | Editor/opener restricted lexer, canonical absolute discovery/revalidation, scrubbed env and direct-exec file-arg boundary are implemented. TUI authorization/TTY handoff and explicit local/remote `!`/`gs` contracts remain open. See [Stage 3 verification](../verification/stage-03.md#m33--editor-and-default-opener). |
...
| ID | 能力 | Stage | 状态 | 验收标准 | 当前证据 |
|---|---|---:|---|---|---|
| OBS-001 | Preview/Jobs/Log 底部抽屉 | 3 | In Progress | 三个页签可独立打开/切换/关闭，保留双栏上下文；异常或等待任务有非侵入提示。 | Bounded normal/narrow layout, Jobs reuse, tab/focus transitions and pane context pass. Log now reads a daemon-owned redacted 1,000-record ring through 256-record pages with validated Job/Endpoint filters; workspace v2 restores drawer tab/focus/rows without bodies. Diagnostic copy and richer Log interaction remain open. See [Stage 3 verification](../verification/stage-03.md#m31--drawer-and-cache-foundation). |
