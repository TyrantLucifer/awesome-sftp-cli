...
| AUTH-002 | 有边界地继承 `~/.ssh/config` | 1 | In Progress | Host/Include/Match/Identity/host-key/auth/GSSAPI/proxy/agent按system ssh生效；SFTP固定关闭TTY/escape/forward/commands/background/tunnel及新GSS delegation但保留ControlMaster兼容，既有master状态是用户边界。Helper与remote `!`/`gs`另强制fresh transport、GSS delegation和ControlMaster/Path/Persist off。 | Host/Identity/host-key/auth/GSSAPI/proxy/agent and conflicting fixed-option coverage is green in Hosted runs; Include/Match and ControlMaster new/reuse evidence remains a final Stage 1 gate. See [Stage 1 verification](../verification/stage-01.md). |
...
| CONN-002 | OpenSSH stdio 上的结构化 SFTP | 1 | In Progress | daemon 按 ADR-0001 的 validated-absolute-ssh-path 与精确 argv 从子进程 stdin/stdout 创建 SFTP client；binary/host 均不可经 PATH 或选项注入，也不解析人类可读 `sftp` 命令输出。 | Structured pkg/sftp Provider passes the shared contract over protocol pipes; real system-sshd Hosted result pending. See [Stage 1 verification](../verification/stage-01.md). |
| CONN-003 | CLI 双路径启动 | 1 | In Progress | `amsftp <left> <right>` 接受本地路径与 `host:path`，两个初始位置均被解析、连接并独立报告错误。 | Local and host-alias absolute-path parsing plus daemon endpoint connection are tested; three-combination Hosted PTY matrix pending. See [Stage 1 verification](../verification/stage-01.md). |
...
| PANE-002 | 任意端点组合 | 1 | In Progress | local/local、local/remote、remote/local、同 remote 双路径和 remote A/remote B 均能浏览。 | Client/daemon supports independent SSH endpoint creation per pane; real five-combination matrix pending. See [Stage 1 verification](../verification/stage-01.md). |
