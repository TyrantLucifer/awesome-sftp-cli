...
| JOB-001 | 持久 Job | Verified | 复制、移动和删除拥有稳定状态、事件、检查点和控制命令；TUI 的有界 Jobs 抽屉按宽度切换左右主从或列表加单行详情布局，每个任务行显示文件身份、四秒滚动平均速度和状态化进度，选中 relay Job 可查看脱敏阶段耗时，删除不使用字节百分比，并只提示实际可用的控制键；成功终态会精确刷新当前可见的受影响目录。 | [Job manager 测试](../../internal/transfer/manager_test.go)、[TUI reducer 与渲染测试](../../internal/tui/model_test.go)、[Jobs drawer 测试](../../internal/tui/render_test.go)、[键位参考](../user/keymap.md)与[CLI 参考](../user/cli.md) |
| JOB-002 | 重启恢复 | Verified | daemon 重启后只跨越安全边界恢复，不把未知结果标记成功。 | [Job journal 测试](../../internal/transfer/job_journal_test.go) |
| COPY-001 | 流式安全复制 | Verified | 先写 part，验证和提交后再暴露最终目标；未限速 SFTP 下载在 Provider 内保持最多 1 MiB、32 请求的有界读取窗口，Worker 仍只按 256 KiB 块消费并至多重叠下一块与当前 durable checkpoint，预读不推进可恢复 offset；限速热更新会从已交付 offset 安全切回精确计量读取；checkpoint 持久保存 read/write/sync/stat/checkpoint 累计阶段耗时用于性能诊断；TUI 多选引用异步冻结期间会按代次排队粘贴，不复用旧剪贴板；同一粘贴产生的多个 Job 在共享 WAL 写入门禁内逐个持久化，不因并发创建丢项。 | [SFTP 请求窗口测试](../../internal/provider/sftp/provider_test.go)、[worker 测试](../../internal/transfer/worker_test.go)、[Job journal 测试](../../internal/transfer/job_journal_test.go)、[relay 基准](../../internal/transfer/worker_benchmark_test.go)、[TUI 多选回归测试](../../internal/tui/model_test.go)、[并发 Job 持久化测试](../../internal/state/jobstore/store_test.go)与[持久传输指南](../user/durable-transfers.md) |
...
| REL-013 | 一键安装与升级 | Implemented | `amsftp upgrade` 在 macOS/Linux 识别 Homebrew 与独立安装；独立 installer 在任何目标变更前预检 executable/config/state/cache 信任链，默认 HOME 不可信时自动复用已预置的 owner-private managed root，缺失时零目标变更并给出一次性管理员初始化命令；managed marker 保持后续启动和升级路径一致。有新版本时升级通过 authenticated RPC 原子拒绝活动 Job、停止旧 daemon、执行渠道升级、仅恢复原本运行的 daemon，并验证精确的新 binary/daemon 版本。owner-private 升级门控覆盖 daemon 探测到最终验证，令其他 TUI 等待后重连而不以旧 executable 抢先恢复，并在任何 mutation 前拒绝第二个升级。Human 模式在各潜在阻塞阶段开始前输出进度并对耗时安装做提示，验证失败区分 binary/daemon 检查并给出安全恢复命令；JSON 模式保持单文档且不输出进度。 | [ADR-0019](../architecture/adr/0019-managed-install-root-preflight.md)、[安装说明](../release/INSTALL.md)、[升级与回滚](../release/UPGRADE.md)、[安装器测试](../../internal/installscript/install_test.go)、[路径测试](../../internal/platform/paths_test.go)、[升级编排测试](../../internal/app/upgrade_command_test.go)、[恢复门控测试](../../internal/app/stage1_test.go)与[渠道测试](../../internal/selfupdate/selfupdate_test.go) |
