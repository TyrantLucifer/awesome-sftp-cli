...
| REL-013 | 一键安装与升级 | Implemented | `amsftp upgrade` 在 macOS/Linux 识别 Homebrew 与独立安装；独立 installer 在任何目标变更前预检 executable/config/state/cache 信任链，默认 HOME 不可信时自动复用已预置的 owner-private managed root，缺失时零目标变更并给出一次性管理员初始化命令；managed marker 保持后续启动和升级路径一致。有新版本时升级仍通过 authenticated RPC 原子拒绝活动 Job、停止旧 daemon、执行渠道升级、仅恢复原本运行的 daemon，并验证精确的新 binary/daemon 版本。Human 模式在各潜在阻塞阶段开始前输出进度并对耗时安装做提示，验证失败区分 binary/daemon 检查并给出安全恢复命令；JSON 模式保持单文档且不输出进度。 | [ADR-0019](../architecture/adr/0019-managed-install-root-preflight.md)、[安装说明](../release/INSTALL.md)、[升级与回滚](../release/UPGRADE.md)、[安装器测试](../../internal/installscript/install_test.go)、[路径测试](../../internal/platform/paths_test.go)、[升级编排测试](../../internal/app/upgrade_command_test.go)与[渠道测试](../../internal/selfupdate/selfupdate_test.go) |
