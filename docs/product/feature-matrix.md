...
| COPY-001 | 流式安全复制 | Verified | 先写 part，验证和提交后再暴露最终目标；同一有界缓冲区至多预读下一块并与当前 durable checkpoint 重叠，预读不推进可恢复 offset；TUI 多选引用异步冻结期间会按代次排队粘贴，不复用旧剪贴板；同一粘贴产生的多个 Job 在共享 WAL 写入门禁内逐个持久化，不因并发创建丢项。 | [worker 测试](../../internal/transfer/worker_test.go)、[relay 基准](../../internal/transfer/worker_benchmark_test.go)、[TUI 多选回归测试](../../internal/tui/model_test.go)、[并发 Job 持久化测试](../../internal/state/jobstore/store_test.go)与[持久传输指南](../user/durable-transfers.md) |
