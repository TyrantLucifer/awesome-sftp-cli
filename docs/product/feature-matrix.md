...
Stage 0 行已按完成证据标记为 `Verified`；Stage 1 的 M1.1–M1.3 已验证，M1.4 的恢复状态机与远端 source-streaming cursor 已实现并通过定向本地测试，真实环境 exact-candidate 证据正在收口。其余 Stage 2–6 行仍按阶段保持 `Planned` 或在跨阶段验收仅完成 Stage 1 slice 时保持 `In Progress`。实现后必须用可复现的实际证据替换预期证据，包括测试名或命令、验证平台、测试夹具/环境和结果记录；只有代码、测试、阶段验证记录、功能矩阵与 `PROJECT_STATE.md` 一致时，状态才能改为 `Verified`。
...
| PANE-004 | 增量目录加载 | 1 | Implemented | 目录项以流式/分页方式进入模型，首屏无需等待完整目录读取，加载可取消。 | 256-entry daemon pages, generation-safe reducer, slow-Fake responsiveness and cancellation pass. ADR-0011's packet-bounded SFTP cursor test proves `Limit=1` returns before a blocked second source batch; full exact-candidate Hosted evidence is pending. See [Stage 1 verification](../verification/stage-01.md). |
