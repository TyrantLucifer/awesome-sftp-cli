...
| XFER-004 | 有界内存与背压 | 2 | In Progress | 读取速度受目标写入背压约束；缓冲总量与并发配置成比例，不随文件大小增长。 | 单文件 worker 使用 Plan 固定 256 KiB buffer，读取后完整处理 short write、Sync 与 checkpoint 才继续；目录队列、并发总预算和资源曲线留在 M2.3。 |
