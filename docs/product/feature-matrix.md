...
| AUTH-007 | 无附着客户端时等待认证 | 2 | Implemented | 任务需要交互认证且没有 TUI 时进入 `waiting_auth`；重新附着后可继续，不进行无限失败重试。 | Transfer Manager classifies `auth_required` separately as durable `waiting_auth`; Resume returns it to queued after system OpenSSH auth is repaired. Existing real auth/reattach Hosted jobs cover broker detach/renewal. |
...
| AUTH-010 | 认证失败可恢复 | 2 | Implemented | 认证失败保留任务与安全检查点，展示端点和 OpenSSH 原因；票据/agent 修复后可手动或策略化重试。 | Auth errors do not discard JobJournal state or part; Jobs exposes waiting reason/recent stable error, and explicit Resume reuses frozen Plan after external credential renewal. |
...
| PANE-009 | 符号链接可见且递归保守 | 2 | Implemented | 链接目标信息可见；递归复制/搜索默认不跟随目录链接，除非用户策略显式允许并有循环保护。 | M2.3 discovery emits links without recursing/copying; M2.4 recursive delete removes only the link and preserves its target. Million-entry/depth and dedicated symlink tests pass; final exact-SHA promotion is pending. |
...
| VIM-006 | `y` 复制引用 | 2 | Implemented | `y` 将选中 Location 引用写入内部剪贴板，不读取文件内容、不立即启动传输。 | Reducer and PTY tests capture single/multi/directory FileRefs through the daemon without a mutation; Planner revalidates before Job creation. |
| VIM-007 | `d` 剪切引用 | 2 | Implemented | `d` 只标记 cut intent，不删除或重命名任何源；状态栏清晰显示待移动项。 | Lowercase `d` shares capture only; source mutation occurs only after confirmed `p`, destination commit, SHA-256 proof and source revalidation. PTY proves the separation. |
| VIM-008 | `p` 创建操作意图 | 2 | Implemented | `p` 以剪贴板和目标栏位置创建不可变 OperationIntent；普通复制直接排队，高风险项进入确认。 | Copy queues one Job per frozen item; cut paste opens a bounded move confirmation. Counted paste caps at 1,024 and all Jobs re-enter Planner/conflict checks. |
| VIM-009 | `D` 执行真实删除 | 2 | Implemented | 只有大写 `D` 进入删除计划与确认；确认内容包含端点、数量、目录递归和可逆性。 | Key/reducer tests distinguish `d`/`D`; `D` captures frozen identities then requires scope and irreversible confirmations. Native PTY proves both prompts and deletion. |
| VIM-010 | 计数前缀 | 2 | Implemented | 支持如 `5j`、`3k` 和适用操作的计数；计数越界有确定截断且不会泄漏到下一命令。 | Reducer bounds navigation at 1,000,000, freezes counted `y`/`d`/`D` runs, caps counted paste Jobs at 1,024 and clears counts after one command. |
| VIM-011 | `.` 重复 | 2 | Implemented | 重复最近一次可重复的高层动作，重新绑定当前 Location 并重新执行能力/风险检查，不复用过期确认。 | Copy repeats frozen intent; cut/rename reopens move confirmation; delete restarts both confirmations. Stale identities are revalidated by Planner. |
...
| VIM-015 | `r` 重命名 | 2 | Implemented | `r` 以当前或单个选中条目创建重命名意图，提交前验证同端点能力、目标冲突与源身份。 | `r` rejects multi/root/same/invalid names, captures one frozen FileRef and creates a durable same-endpoint move. PTY rename and atomic/fallback tests pass. |
...
| DAEM-004 | TUI 退出后任务继续 | 2 | Implemented | 最后一个 TUI 退出后，非交互任务继续运行；需要交互时进入等待状态。 | Manager owns a daemon context, not request context; client-cancellation test completes the Job, and PTY exits/reattaches to the same durable Jobs store. |
| DAEM-005 | 事件流与重放游标 | 2 | Implemented | 客户端按游标接收任务/连接事件；短暂断连后可补齐或用一致快照恢复，不重复执行命令。 | Monotonic Job events are queried by after-sequence with bounded limits; request dedup and snapshot/Jobs reload tests prevent command replay across daemon restarts. |
...
| DAEM-007 | daemon 重启恢复 | 2 | Implemented | 异常退出后重新启动可恢复未完成任务、缓存租约和连接需求，并先验证外部状态再继续。 | M2.1 state recovery plus M2.2 part, M2.3 directory and M2.4 commit→delete restart fixtures prove conservative pause, external revalidation and explicit resume. |
...
| ID | 能力 | Stage | 状态 | 验收标准 | 当前证据 |
|---|---|---:|---|---|---|
| JOB-001 | 持久化 Job | 2 | Implemented | 每个写操作在执行前生成稳定 Job ID，并将意图、冻结计划、阶段、进度、决策和检查点事务化写入 SQLite。 | Copy, move, rename and delete all call Manager create APIs that atomically persist V1 Plan/Job/steps/event before scheduler enqueue; no raw mutation RPC exists. |
...
| JOB-003 | 等待与终止状态 | 2 | Implemented | 支持 `paused`、`waiting_auth`、`waiting_conflict`、`retry_wait`、`failed`、`canceled` 和 `completed_with_source_retained`，每个状态记录原因和允许动作。 | Real transfer errors/actions enter each state; Jobs renders waiting/terminal/recovery/error summaries, and restart tests cover legal resume/cancel paths. |
| JOB-004 | 不可变操作计划 | 2 | Implemented | 执行开始后源集合、目标、冲突/完整性策略和初始路由被冻结；变更要求新建或显式修订计划并留痕。 | FileRef, descriptors, capability revisions, route, SHA-256, conflict, part and all memory/discovery budgets are persisted and cross-checked on DecodePlan. |
| JOB-005 | 普通复制立即排队 | 2 | Implemented | 无覆盖、删除或直传风险的复制由 `p` 直接进入 queued，不要求冗余确认。 | Copy PTY presses `y`, Tab, `p` and observes queued/completed without redundant confirmation; racing final still opens a durable conflict. |
| JOB-006 | 高风险计划确认 | 2 | Implemented | 覆盖、递归删除、不可逆删除、跨端点移动和直传在执行危险阶段前展示源/目标/数量/策略/可逆性。 | Cut paste shows frozen move confirmation; overwrite waits on conflict; delete shows scope plus irreversible confirmation. Direct is not a Stage 2 route and cannot be selected. |
| JOB-007 | 队列与有界并发 | 2 | Implemented | daemon 在配置的全局/端点并发上限内调度，公平推进多个任务，不能无限创建 goroutine、文件句柄或 SSH 连接。 | Manager uses fixed worker and queue caps (runtime 4/128; tests 1), acquires frozen endpoint leases before enqueue and bounded directory streams; saturation/control tests pass. |
| JOB-008 | 暂停、继续与取消 | 2 | Implemented | 用户可暂停、继续或取消支持的任务；动作在安全检查点生效并明确当前阶段是否可中断。 | Durable controls, post-close checkpoint refresh, cancel-part retention, event replay and `J` P/U/C reducer/IPC tests pass. |
...
| ID | 能力 | Stage | 状态 | 验收标准 | 当前证据 |
|---|---|---:|---|---|---|
| XFER-001 | 内部 clipboard 保存 Location 引用 | 2 | Implemented | `y`/`d` 保存 Endpoint ID、规范化路径、当时元数据与意图；粘贴时重新验证源，不持有完整内容。 | Clipboard holds immutable FileRef Location/kind/fingerprint/capability revision for single/multi/directory; stale source tests reject changed/deleted identity. |
| XFER-002 | 本地与远端任意方向复制 | 2 | Implemented | local→local、local→remote、remote→local、同 remote 和 remote A→remote B 都通过同一 Job 模型完成。 | Local and bidirectional SFTP PTY plus same-remote and independent remote A→B real-sshd directory fixtures passed exact M2.2/M2.3 Hosted runs. |
| XFER-003 | remote A→B 默认本机中继 | 2 | Implemented | 不满足服务端路径时，daemon 使用固定大小缓冲从 A 读向 B 写，默认不把完整文件落本地磁盘。 | Two independent sshd sessions relay through a 7-byte test buffer with no cache/full local file; exact M2.3 Hosted runs passed. |
| XFER-004 | 有界内存与背压 | 2 | Implemented | 读取速度受目标写入背压约束；缓冲总量与并发配置成比例，不随文件大小增长。 | Plans freeze 256 KiB stream plus 64 queue/256 page/128 depth; 100 GiB synthetic stops at 64 KiB checkpoint and million-entry queue stays at 17. |
| XFER-005 | 目标临时 part | 2 | Implemented | 新内容先写目标目录中的唯一 `.part-<job-id>`；未验证完成前最终目标路径不出现新内容。 | Same-directory unique part, ownership/checkpoint identity, final-absent callback, part-open failure and pause/cancel/restart tests prove no partial final. |
| XFER-006 | 验证后原子提交 | 2 | Implemented | 完成大小与策略要求的完整性验证后，使用端点支持的安全 rename 提交；无法保证原子时计划明确标示并采用保守流程。 | Worker rereads part SHA-256, rechecks conflict, conditionally commits and rereads final; LocalFS/SFTP and response-lost real/fake tests pass. |
| XFER-007 | 断点与检查点 | 2 | Implemented | 可恢复端点记录已确认偏移、源版本和 part 身份；恢复前验证三者仍匹配，不匹配则重启或等待决策。 | Durable offset/source/part/SHA state resumes after disconnect/SQLite reopen; close-time fingerprint and abrupt restart revalidation tests pass. |
| XFER-008 | 完整性策略 | 2 | Implemented | 基线至少验证大小和传输边界；有 hash 能力时可选择强哈希；策略和实际验证证据记录到 Job。 | V1 freezes `stream_sha256`; actual checksum is recorded after source-size, part-hash and committed-final-hash proof. No weaker path is advertised. |
| XFER-009 | 目录递归复制 | 2 | Implemented | 递归遍历流式产生子任务，保留相对结构；权限错误和部分失败按条目记录，不将整树预载入内存。 | Million-entry streaming, nested layout, bounded manifest, selective retry and daemon restart tests pass; exact M2.3 Hosted runs are green. |
| XFER-010 | 目录创建与元数据策略 | 2 | Implemented | 目标目录按需创建；权限/时间等元数据只按显式支持与策略保留，不宣称无法保证的所有权语义。 | LocalFS/SFTP workers create roots/descendants on demand and intentionally claim no ownership/time preservation beyond Provider entries. |
| XFER-011 | 短读短写安全处理 | 2 | Implemented | read/write 返回部分字节时正确推进或失败，不跳过数据、不把短写当成功。 | Bounded read/write-all loops and Fake short-I/O effects advance only actual bytes; disconnect/resume digest checks pass. |
| XFER-012 | 空间预检与运行期磁盘满 | 2 | Implemented | 可获取空间时在计划中预检；运行期 ENOSPC 保留可恢复状态且不提交最终文件。 | Stage 2 providers expose no trustworthy free-space capability, so no preflight is guessed; injected ENOSPC/EDQUOT becomes resource-exhausted retry_wait and final stays absent. |
| XFER-013 | 单文件与批量进度 | 2 | Implemented | Job 展示已处理字节/条目、总量已知性、当前文件和阶段；未知总量不伪造百分比。 | JobView derives durable bytes/items/phase/current paths and optional total; renderer prints a ratio only when total is known, otherwise trusted bytes. |
| XFER-014 | 路由计划可解释 | 2 | Implemented | Planner 输出选择的 local、SFTP relay、same-endpoint 或 direct 路径及能力依据，用户可在执行前查看。 | Immutable Plan persists local/SFTP relay and move strategy/capability. Direct is unavailable in Stage 2; Jobs exposes source/final/phase and terminal route outcome. |
| XFER-015 | 失败条目可重试 | 2 | Implemented | 批量任务可只重试失败/跳过条目，已验证成功项不会重复覆盖；重试继承或重新确认相关策略。 | Permission-denied child enters retry_wait; resume content-revalidates its successful sibling and copies only the missing item. |
| XFER-016 | 传输结果清单 | 2 | Implemented | 任务完成时记录每项源、目标、字节、提交方式、验证级别和异常，不仅提供汇总百分比。 | Bounded 256-item source/destination/status/bytes/error manifest plus complete aggregates/truncation is persisted and rendered through Job summary. |

## 16. 移动与重命名

| ID | 能力 | Stage | 状态 | 验收标准 | 当前证据 |
|---|---|---:|---|---|---|
| MOVE-001 | 同端点优先原子 rename | 2 | Implemented | 同一端点且能力允许时使用 rename；跨文件系统或服务器拒绝时不误报成功，转为明确的 copy-delete 计划需重新评估风险。 | Explicit frozen `atomic_rename` selects zero-stream Rename; `Atomic=true`, source absence and final fingerprint are mandatory. Otherwise Plan freezes copy-delete. |
| MOVE-002 | 跨端点 copy→verify→delete | 2 | Implemented | 跨端点移动先完成目标复制、验证和提交，之后才进入源删除阶段。 | File/directory tests assert destination SHA-256/per-item verification and commit precede frozen source revalidation and conditional deletion; PTY confirms cut/paste. |
| MOVE-003 | 验证不足保留源 | 2 | Implemented | 无法达到移动要求的验证级别时不删除源，任务以 `completed_with_source_retained` 或等待决策结束。 | Directory mismatch/symlink, capability loss and unproved delete retain source and persist `move_reason` in terminal event/Jobs summary. |
| MOVE-004 | 源变化阻止删除 | 2 | Implemented | 复制后源版本发生变化时，源删除前检测并保留源；不删除与已复制版本不同的对象。 | `TestManagerMoveRetainsSourceWhenItChangesAfterCommit` mutates the source after final commit and proves retained source plus verified destination. |
| MOVE-005 | 部分移动结果可解释 | 2 | Implemented | 批量移动逐项记录已移动、已复制但保留源、未复制和失败，重试不会重复删除。 | Each selected source becomes its own durable Job; directory manifests are bounded per item, terminal state distinguishes source-retained, and restart postconditions prevent repeat deletion. |

## 17. 删除与回收

| ID | 能力 | Stage | 状态 | 验收标准 | 当前证据 |
|---|---|---:|---|---|---|
| DEL-001 | `D` 专用删除入口 | 2 | Implemented | Normal/Visual 中只有显式大写 `D` 创建删除意图；`d` 始终是 cut。 | tcell/reducer and native PTY prove lowercase capture-only versus uppercase prepare/confirm/create-delete routes. |
| DEL-002 | 删除计划确认 | 2 | Implemented | 计划列出端点、路径/数量、目录递归、trash 能力和不可逆项；取消不产生写操作。 | Frozen target count/first path and irreversible warning are rendered before two Enter actions; Escape clears pending scope with zero mutation. |
| DEL-003 | 有能力时优先 trash | 2 | Implemented | LocalFS 或 helper 声明可靠 trash 时可移动到回收位置并记录恢复信息；能力不确定时不伪装成可恢复。 | Optional `TrashProvider` is usable only with exact frozen `trash` capability; positive test avoids irreversible flag. Current LocalFS/SFTP honestly advertise none. |
| DEL-004 | 纯 SFTP 不可逆警示 | 2 | Implemented | Level 0 SFTP 删除明确显示不可逆，必须完成确认后才执行。 | No-trash Plan requires `IrreversibleConfirmed`; TUI second modal says irreversible and native PTY proves it gates the Job. |
| DEL-005 | 递归删除有界执行 | 2 | Implemented | 递归删除流式遍历并逐项记录；符号链接默认只删链接本身，取消后给出已删/未删清单。 | Frozen page/depth budgets drive descendant-validated bottom-up removal; symlink target preservation and depth bounds pass. Aggregate result counts are durable. |
| DEL-006 | 删除审计与后置检查 | 2 | Implemented | 每项删除记录执行结果；连接在响应前中断时重新 stat 判断后置状态，不盲目重复。 | Delete Plan has verify/delete/verify-absent steps and terminal event; explicit and move response-lost tests prove not-found before success. |

## 18. 冲突与覆盖

| ID | 能力 | Stage | 状态 | 验收标准 | 当前证据 |
|---|---|---:|---|---|---|
| CONF-001 | 目标存在冲突检测 | 2 | Implemented | 计划和提交前均检测目标存在/变化；陈旧预检不能授权静默覆盖。 | Planner preflight plus post-verify commit recheck; racing-final tests cover ask/skip/overwrite/auto-rename without stale authorization. |
| CONF-002 | 明确冲突策略 | 2 | Implemented | 支持跳过、覆盖、重命名/另存为和取消；目录合并与文件覆盖分别呈现。 | Worker/Manager implement ask, skip, overwrite, auto-rename and cancel/pause retention for file and per-item directory Jobs. |
| CONF-003 | Job 级 apply-all | 2 | Implemented | “对本任务全部应用”只影响当前 Job 和匹配的冲突类型，不写成无意的全局默认。 | Persisted conflict rows bind scope to Job/class; J keys W/X/A set apply-all only on selected Job, with isolation tests. |
| CONF-004 | 无客户端时等待冲突 | 2 | Implemented | 后台任务遇到未决冲突进入 `waiting_conflict` 并保留安全状态；附着 TUI 后继续。 | Commit-time conflict persists Job/part and waiting state independent of client; reattached J resolves then resumes frozen Plan. |
| CONF-005 | 覆盖仍使用临时提交 | 2 | Implemented | 覆盖策略不直接截断最终目标；新内容验证成功后再以端点安全流程替换。 | Existing final remains until complete part SHA-256; replacement requires frozen expected destination fingerprint. |
| CONF-006 | 决策留痕 | 2 | Implemented | 每个冲突记录检测证据、用户/策略决策、作用范围和最终结果，日志默认不包含内容。 | V1 conflict row records frozen source/destination/class/resolution/scope; transactional resolution event and secret scan pass. |

## 19. 故障恢复

| ID | 能力 | Stage | 状态 | 验收标准 | 当前证据 |
|---|---|---:|---|---|---|
| RECV-001 | 网络中断恢复 | 2 | Implemented | 读写中断进入可恢复状态，重连后验证源/part/目标再从安全偏移继续或重新开始。 | Disconnect fixture persists offset, closes/reopens endpoint, verifies source/part and completes the exact digest. |
| RECV-002 | daemon kill -9 恢复 | 2 | Implemented | 在 draft、running、verifying、commit 和 delete-source 阶段 kill -9 后，重启不产生损坏最终文件或重复源删除。 | M2.1 process-death transitions plus part, directory and commit→delete manager-stop fixtures cover conservative restart/postcondition reconciliation; native workflows rerun in Hosted. |
| RECV-003 | 仅幂等阶段自动重试 | 2 | Implemented | read、临时写等可证明幂等阶段可退避重试；rename、提交、删除先检查后置状态。 | Read/write resume uses checkpoint; commit/rename/delete response-lost tests stat postconditions before any re-execution. |
| RECV-004 | 退避与重试预算 | 2 | Implemented | 可重试错误使用有上限的指数退避与抖动；不可重试错误立即失败，用户可手动重试。 | Stage 2 deliberately has no blind automatic retry loop: retryable faults enter durable retry_wait with a one-minute earliest retry and explicit Resume; non-retryable errors fail immediately. This is the bounded v1 policy. |
| RECV-005 | 取消默认保留恢复材料 | 2 | Implemented | 取消默认保留匹配的 part 和检查点，并显示占用与清理动作；策略禁止时在取消前说明会清理。 | Cancel control closes/syncs/restats and retains matching part/checkpoint; terminal summary and test assert retained recovery evidence. Explicit orphan cleanup is deferred rather than guessed. |
| RECV-006 | part 身份校验 | 2 | Implemented | 恢复仅使用属于同一 Job、目标和源版本的 part；孤儿或冲突 part 不被误接续。 | Job-specific canonical part path plus persisted fingerprint/size/source/final bindings are cross-checked; mismatch rejects resume. |
| RECV-007 | 磁盘满、权限与短写保持目标安全 | 2 | Implemented | ENOSPC、EACCES 和短写使任务失败/等待，但最终目标不出现未验证的新内容，错误包含失败阶段。 | Permission/resource/short-I/O tests prove typed state and absent unchanged final; real XFS ENOSPC also covers the state store transaction boundary. |
| RECV-008 | 恢复证据可审计 | 2 | Implemented | 恢复记录原失败、重连/重试次数、验证判断、恢复偏移和最终提交；用户可在 Jobs/Log 查看摘要。 | Monotonic events and checkpoint payloads retain phase/offset/outcome; JobView exposes recent stable error and `job_recovered` reason. |
...
| ID | 能力 | Stage | 状态 | 验收标准 | 当前证据 |
|---|---|---:|---|---|---|
| DIRECT-001 | 同端点 rename 快速路径 | 2 | Implemented | 移动在同端点且 rename 语义满足时优先原子 rename，无需传输内容。 | Capability test advertises `atomic_rename`, proves zero OpenRead/OpenWrite calls, conditional Rename, atomic result and both postconditions. |
...
| SCALE-004 | 有界工作队列 | 2 | Implemented | 递归生产者受消费者和持久队列背压；队列容量可配置并有安全默认，不因深树耗尽内存。 | Discovery freezes 64 queue items, 256 entries/page and depth 128 by default; the million-entry fixture proves capacity 17/page 31 hard ceilings and exact M2.3 Hosted runs pass. |
...
| SEC-006 | 路径规范化与根边界 | 2 | Implemented | 所有操作使用 Endpoint+规范化路径；遍历中的 `..`、绝对跳转和链接不会越过用户确认的根范围。 | Provider normalization, rooted LocalFS handles, descendant validation, same-tree rejection and symlink no-follow tests enforce Endpoint/root boundaries. |
| SEC-007 | 临时目标与符号链接竞态防护 | 2 | Implemented | part 名不可预测且绑定 Job；提交前重新验证目标类型/身份，拒绝把预期文件写到被替换的链接/目录。 | Random Job ID binds same-directory part; conditional open/rename and final-race/symlink tests reject replacement identities. |
...
| SEC-013 | 破坏性操作显式授权 | 2 | Implemented | 不可逆删除、覆盖、跨端点移动源删除和直传危险阶段不能由默认焦点误触；确认可取消且作用域明确。 | Cut paste confirms move; D freezes scope then requires irreversible confirmation; overwrite waits conflict; count/dot reconfirm. Direct route is unavailable. |
...
| OBS-002 | 任务阶段与等待原因 | 2 | Implemented | Jobs 显示状态、当前阶段、源/目标、进度和 `waiting_auth`/`waiting_conflict`/retry 等原因及允许动作。 | Bounded J view renders state, phase, source/final, items, bytes/total, wait reason, summary, recent error/recovery and P/U/C/conflict keys. |
| OBS-003 | 速度、吞吐与 ETA 诚实展示 | 2 | Implemented | 有可靠总量才显示百分比/ETA；瞬时与平均速度有采样窗口，未知值显示 unknown 而非 0 或 100%。 | Stage 2 minimal Jobs renders only durable bytes and an optional known total; it never invents speed, ETA, 0% or 100% from absent samples. Sampled throughput/ETA visualization remains part of the Stage 3 full drawer. |
| OBS-004 | 结构化连接与任务事件 | 2 | Implemented | 事件至少包含时间、级别、端点、Job、阶段、稳定错误码和脱敏消息，可按 Job/端点过滤。 | V1 job_events are per-Job monotonic/bounded and carry stable phase/outcome/error code; persistent logs allow only registered endpoint/Job/error fields. |
...
| OBS-008 | 验证与提交证据 | 2 | Implemented | 完成任务可查看实际大小/hash 策略、part、提交方式、源删除判定和异常保留源原因。 | Frozen verification plus terminal bytes/items/SHA/outcome/manifest/move_reason are durable; Jobs shows completion/source-retained summary. |
...
| PLAT-007 | 文件系统语义差异 | 2 | Implemented | 大小写、权限、rename 原子性、链接和时间精度按端点能力处理，不把 macOS/本地语义强加给远端。 | Planner uses Provider capabilities/normalized Location only; UTC fingerprint canonicalization, LocalFS/SFTP contracts, symlink and explicit atomic-rename gating pass cross-platform. |
