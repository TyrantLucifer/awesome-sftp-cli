...
Production Helper distribution is **CLOSED** in Stage 4. The shipped production verifier trusts no fixture key, and ordinary runtime/configuration cannot enable the repository's non-release test fixture. Consequently there is no supported production install command or production Helper download in this stage. Lifecycle tests use explicit non-release test-only injection; they are not a production artifact or trust claim.
