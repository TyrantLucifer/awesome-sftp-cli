...
| `c` | connect/switch Endpoint | remappable |
...
Digits remain count prefixes before keymap lookup. Counts are accepted only for the existing bounded navigation/copy/cut/paste/delete/rename set. Unsupported count/action combinations are ignored safely. Visual mappings use the `visual` context; confirmation, text-entry, authentication, recovery, and drawer-specific keys retain their fixed safe behavior.
