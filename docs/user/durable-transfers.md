...
The daemon applies one shared bandwidth/resource scheduler. A Plan may freeze a per-Job byte rate; global, both Endpoint, and Job token buckets are enforced at a fixed maximum 256 KiB quantum with bounded interactive service. Fast paths that cannot be rate-controlled are skipped when the policy requires exact control. The SFTP read window reserves its 1 MiB buffer and 34 supporting goroutines in the same resource ledger before execution. Runtime configuration may tighten, but never expand, the documented hard ceilings (16 active Jobs, 8 per Endpoint, 512 queued Jobs, 32 connections, 4 per Endpoint, 32 SSH processes, 4 Helper processes, 512 FDs, 256 goroutines, and 64 MiB accounted memory).
...
The default Jobs list prioritizes user-facing operation, state, and progress. Byte counts automatically use `KiB`, `MiB`, `GiB`, and larger units, with a percentage when the total is known. During transfer, a bounded four-second moving window smooths the displayed rate across the 256 KiB durable-progress steps; the first sample shows `—/s`, while waiting, paused, and terminal Jobs omit a live rate. At most 16 samples are retained per visible Job.

Selecting a relay Job expands its source and destination onto separate lines and shows cumulative `read`, `write`, `sync`, `stat`, and `checkpoint` stage time once data has moved. The same path-free counters are available in the Job JSON `performance` object as nanoseconds. Read may overlap sync and checkpoint, so stage values are diagnostic evidence and must not be added together as total elapsed time. Narrow terminals preserve both ends of each path with an explicit middle ellipsis so long paths do not displace state and progress.
