...
- **状态**：Complete
- **收口进度**：40/40 Feature Matrix 行已验证；完整本地门禁、candidate-tree 扫描、独立 cold-start audit 与 exact-SHA Hosted 交付记录见 Stage 3 Verification/PR #3
- **阶段类型**：日常使用体验闭环
...
**里程碑进度**：Complete — drawer、live admission/LRU/lease、dead-preview reclaim、crash-safe cleanup 与 exact pinned-offline reopen 均已验证。
...
**里程碑进度**：Complete — bounded render/range/external fallback、zero-read object metadata 与真实 Kitty 均已验证。
...
**里程碑进度**：Complete；provider-stream SHA、no-replace original preservation、Vim/Neovim 和平台 opener 均纳入验证矩阵。
...
**里程碑进度**：Complete；local/temporary-sshd `!`+`gs` harness、CI provisioning、TTY 恢复与 bounded cleanup 均已验证。
...
这里的 checkbox 表示已经有满足该条完整验收范围的证据；完整命令、平台、candidate-tree 扫描、cold-start audit 与 exact-SHA Hosted 交付记录见 [Stage 3 Verification Record](../verification/stage-03.md) 和 PR #3。

- [x] Preview/Jobs/Log 抽屉在正常、窄屏和窗口调整时保持可用焦点模型。`internal/tui/drawer_test.go` 覆盖 open/switch/refocus/close、lowercase separation 和 `100x16`/`32x7` bounded snapshots；workspace v2 只恢复结构状态。
- [x] 文本、代码、JSON、未知二进制和大文件预览均有有界读取与明确的部分内容提示。Planner/render/TUI tests 冻结 64 KiB read、512 KiB retain/output、256 KiB/depth-64 JSON、10,000 lines，并以 100 GiB fixture 证明 range/early rejection；file/directory/symlink metadata 还断言 0 content read/0 materialization。
- [x] 至少一种支持的终端图片协议验证通过；不支持时无乱码并安全回退。真实 Kitty 0.47.4 active probe 收到 exact `i=31;OK` 并输出 bounded 131 bytes；Kitty/iTerm2/Sixel/none/corrupt/oversize fixtures 证明 proof-gated output 与 safe fallback。
- [x] 缓存配额、LRU、固定项、内容去重和活跃租约行为有确定测试。Manual-clock quota/lease、concurrent SHA-256 dedup、live bytes/object/workspace admission、short-read cleanup、dead-preview-only reclaim、exact pinned-offline restart/disconnect/reconnect/ambiguity 和 explicit safe-clear tests 通过。
- [x] 编辑无变化不上传；仅本地变化需确认；双方变化必进入冲突；判断不确定也不覆盖。Local/remote 全矩阵覆盖 modify/delete/replace/stat unknown；provider-stream SHA 进一步证明 same-size/same-second、same-fingerprint 内容改写仍进入 conflict。
- [x] 编辑上传复用 Stage 2 Job，并在失败/重启后保留可恢复本地副本。Atomic session→Plan/Job binding、completion/failure/restart tests 通过；Plan V2 冻结 deterministic preservation sibling，Provider 先 no-replace 移走原文件并有界验证，再 no-replace 发布 part；任一竞态/不确定状态都保留可达 bytes 并进入 conflict/recovery。
- [x] editor优先级/受限env lexer/PATH发现后absolute direct exec及macOS`/usr/bin/open`、Ubuntu`/usr/bin/xdg-open`通过；路径始终独立absolute arg，立即返回opener不导致过早逐出。Resolver/Plan、opener lease、本地真实 Vim/macOS open 与 Hosted Vim/Neovim/xdg-open 矩阵均纳入交付门禁。
- [x] `!`覆盖local Dir+shell arg与remote exact fresh argv、byte-safe cwd/marker-before-input、32KiB、1MiB双stream持续drain、cancel/detach unknown/refresh；shell/路径失败发送0 command bytes。Unit contract、single-flight、Esc cancel、15-minute timeout 和 local/temporary-sshd real harness 均通过。
- [x] `gs`覆盖home无command/current-cwd probe+显式fallback、`-tt`、TTY/foreground pgrp/SIGWINCH，以及正常/非零/信号后termios/alternate/cursor、两栏与Job正确恢复。Unit、真实 local PTY 和 temporary-sshd harness 通过；post-spawn foreground failure 还证明 terminate→reap→resume，单一 stalled cleanup 未完成前新 handoff fail closed。
- [x] 日志、缓存元数据和配置中无认证秘密或无界文件内容。Strict manifest/catalog、bounded redacted ring/diagnostic、inherited real auth/Kerberos secret scans 与 final candidate secret/pollution inventory 均纳入交付记录。
- [x] Helper key custody/恢复、第一组 public key、撤销窗口与双 key 轮换演练有可审查证据；否则 Stage 4 Helper 分发保持关闭。当前选择本条允许的 fail-closed 分支：仓库只有 public RFC test vectors、deterministic key-ID verifier、custody/recovery runbook 和 public-only tabletop；真实 production custody/key/rotation 未发生，因此 [Helper distribution gate](../security/helper-trust-handoff.md) 明确 **CLOSED**，不授权下载/上传/安装。
