...
最新增量：reviewed third-party NOTICE SHA `197ff6f12135be670df40bd5f0dfe7e8c72fe483` 将 20 个 runtime modules 的 declared/resolved/replacement/SPDX identity 与 exact redistribution source bytes 一一绑定；除全部 root `LICENSE` 外，完整包含 `modernc.org/libc/LICENSE-3RD-PARTY.md` 和 `modernc.org/memory` 的 `LICENSE-GO`/`LICENSE-MMAP-GO`。`make notice-check` 解析真实 module graph，拒绝缺失/额外/重复/漂移、symlink/unsafe source、hash/byte 变化与 committed NOTICE 差异；CI 在打包前重新生成并 byte-compare。exact current/oldstable 本地门禁均首轮通过。push [29641186074](https://github.com/TyrantLucifer/awsome-sftp-cli/actions/runs/29641186074) quality 完整通过；其唯一 native failure 是既有 exact-stderr-cap reader race。PR [29641186949](https://github.com/TyrantLucifer/awsome-sftp-cli/actions/runs/29641186949) quality 在生成/比较/打包 NOTICE 后才于冻结 Stage 5 daemon 的既有 readiness 轮询失败，同 SHA push quality 提供成功 companion。未 rerun、未弱化门禁。该切片技术关闭第三方 redistribution NOTICE，但不替 owner/legal 选择项目 LICENSE；真实 public channel、final release trust 与 production Helper/Level 2 仍开放。
