...
时序稳定性增量：SHA `60255dc8dc5e4732ae0fe55ac11ca860390bc81c`、tree `27fe962f3c712eae16e061a06d8460542eaaedf5` 的四条 RED/GREEN 切片为 Helper client/process/stderr 终态、tail 两次轮询和 Stage 2 PTY delete 选中态建立显式同步屏障。tail 仍保留原 80 ms 限制，PTY 仍保留原等待上限；没有弱化 assertion。Helper/tail 各 200 轮、真实 Stage 2 PTY 10 轮与精确双 Go完整门禁通过。push `29659577751` 的 unchanged local-command 20 ms start-cancellation failure 有同 SHA PR quality 成功 companion；PR `29659579470` 暴露了 introduced immediately-exiting exact-stderr child 的 stdout-close handshake race，同 SHA push oldstable Intel 通过。纠正恢复 resident child，并在读取 diagnostic 前通过新 terminal barrier 关闭 session；双工具链各 200 轮及最终双 Go完整门禁通过，纠正 SHA Hosted 待验证。该批不生成 RC evidence，不改变 production Helper/Level 2 CLOSED 边界。
