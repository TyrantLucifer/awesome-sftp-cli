...
时序稳定性增量：SHA `60255dc8dc5e4732ae0fe55ac11ca860390bc81c` 建立 Helper 终态合流、确定性 tail poll 与初始 Stage 2 屏障；SHA `573aeefcaa488d73a7b5186a46ef03f87c1d8416` 恢复 resident stderr child；SHA `60ff9080812e0e47f720d102671116a6557b9921` 保证 65,536/65,537 精确写入。Loaded-selection SHA `2f4ed536ca2553e1bd266ed66fd275b664a00690` 让四条选中项动作共用屏障并通过双 Go 本地门禁，但 Hosted 发现固定 `READ-ONLY | sort:name` 不适用于含 capability 的远端 loaded 行。当前纠正新增单次 final-screen required/absent 查询：最终屏幕必须含目标且不含 `READ-ONLY | loading`；确定性 local/remote capability screen、完整本地 PTY current race 20 轮、exact oldstable 10 轮与最终双 Go 完整门禁均通过。临时 sshd 压力仅在 isolated transport 不再接受连接后失败，保留分类且不重跑。该批不生成 RC evidence，不改变 production Helper/Level 2 CLOSED 边界。
