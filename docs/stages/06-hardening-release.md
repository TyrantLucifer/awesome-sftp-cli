...
最新增量：no-argument picker SHA `fbddcaa0208d2fca5c285162c5c9ffbf7fa8d318` 在 fresh owner-private HOME 直接以空参数启动 canonical archive binary，展示 SSH-config aliases，选择独立 chroot `recovery-picker` 并看到其 marker；保存双远端 workspace 后，再次空参数启动、筛选并重开 workspace。两套 `auth-integration` 都先验证同运行 SHA-bound bundle，再通过 OpenSSH/TUI 与 Kerberos。PR [29638718437](https://github.com/TyrantLucifer/awsome-sftp-cli/actions/runs/29638718437) 24/24 全绿；push [29638717696](https://github.com/TyrantLucifer/awsome-sftp-cli/actions/runs/29638717696) 的 quality/auth 全绿，唯一 oldstable macOS Intel failure 是既有 Helper heartbeat visibility fixture 且同 SHA PR companion 成功，未 rerun。它关闭 preview archive 的显式连接和无配置 host/workspace picker 演练，不是真实公开 channel；pinned-cache upgrade/failed-upgrade recovery、final release trust 与 production Helper/Level 2 仍开放。
