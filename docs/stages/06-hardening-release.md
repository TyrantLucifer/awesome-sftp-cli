...
时序稳定性增量：基于 `c81887a8fcfacd488ff88c23ead46f34f2f972e5` 的四条 RED/GREEN 切片为 Helper client/process/stderr 终态、tail 两次轮询和 Stage 2 PTY delete 选中态建立显式同步屏障。tail 仍保留原 80 ms 限制，PTY 仍保留原等待上限；没有弱化 assertion。Helper/tail 各 200 轮、真实 Stage 2 PTY 10 轮、focused oldstable/vet/lint/docs 与精确双 Go 完整本地门禁均通过。该批属于 pre-RC process/session 与 Hosted-fixture hardening，不生成 RC evidence，不改变 production Helper/Level 2 CLOSED 边界。
