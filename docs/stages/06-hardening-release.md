...
当前增量：foundation SHA `ae172d9bb419bc6b0b05cf3ef3467258496d463f` 冻结 shared export policy 与 deterministic bounded support-bundle core；doctor implementation SHA `7aa6f20d1cada28064cfd4f169442ef219a9b1a8`、tree `df6450899ae9316600d38f72f426f3a4c308ce91` 已交付公开 CLI、human/JSON、help/man/三种静态 completion 与真实只读 probes。config/path/socket/daemon/OpenSSH/known-host/database/cache/Helper/disk/optional endpoint 按固定顺序输出稳定 token，raw cause、endpoint metadata、absolute path 和 subprocess output 不导出。命令不创建应用目录、不 repair/migrate DB、不启动 daemon、不改 Helper、不提示 credential、不上传；stopped sidecar-free DB 才用 immutable compiled-head validation，live DB 只 warn。`ssh -G -T` 每流 64 KiB，重复安全字段/未知 host-key policy fail closed；直连只做 5 秒无 payload TCP connect，proxy 明确为 not probed。focused normal/race/vet、isolated-HOME no-creation smoke、exact current `make ci` 与 oldstable `make check` 通过；Hosted exact-SHA 尚待。此前 docs SHA `20bc338` push 24/24，PR 两个既有 stderr-reader/PTY-selection failure 均有同 SHA push companion，未 rerun。support-bundle real composition/CLI/private no-replace publication/cleanup、corrupt-state、完整 secret matrix、兼容矩阵与 finding ledger 仍开放。
