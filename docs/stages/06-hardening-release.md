...
当前增量：foundation SHA `ae172d9bb419bc6b0b05cf3ef3467258496d463f` 冻结 shared export policy 与 deterministic bounded support-bundle core；doctor implementation SHA `7aa6f20d1cada28064cfd4f169442ef219a9b1a8` 已交付公开 CLI 与真实只读 probes。docs SHA `d345c362ad8b6ea8c67b09a4e351ad27e929965a` 的 push/PR 已完整分类：只命中既有 Helper heartbeat/stderr reader visibility timing，same-SHA companion/step 和全部新包证明无 doctor/statefs/OpenSSH inspection/CLI/docs/packaging/auth 回归，未 rerun。support-bundle SHA `bfc59c8c0478a7043b8287f52d31dd3477b30554` 进一步交付 public preview/create、manifest+八个 reviewed source、exact SHA consent、create 时重新组合、deterministic bounded archive、owner-private 0600 no-replace/no-follow publication、file/parent sync 与 cancellation/failure cleanup。配置只导出 shape；diagnostic/Job 的 message、ID、path、argv、location、error、content 均不进入 bundle，unsafe metadata 统一 redacted；daemon 缺失/失败只产生安全空摘要，不启动 daemon、不连接 endpoint、不提示或上传。focused normal/race/vet/lint、isolated-HOME no-creation real preview、exact current `make ci` 与 exact oldstable `make check` 均通过；本批 Hosted 尚待。corrupt-state/完整跨系统 secret matrix、兼容矩阵与 finding ledger 仍开放。
