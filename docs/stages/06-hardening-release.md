...
最新增量：preview-handoff SHA `e61b28441996bc50846d45c2833774748f323e71` 由 `quality` 上传本次运行构建的七文件 public-preview bundle，`auth-integration` 以同一 SHA 绑定的 artifact 名称下载，并在解压 canonical Linux archive 前验证 exact 文件集、`checksums.txt` 和 `VERSION.json` commit。fresh-user OpenSSH authentication、Stage 1 TUI recovery 与 MIT Kerberos/GSSAPI 全部只运行归档内 binary；PR 24/24 全绿，push 唯一 macOS Intel stderr hard-cap 时序失败由同 SHA PR companion 成功分类，未 rerun。它关闭的是显式 location 的 preview archive→SSH/TUI 交接切片，不是无参数 host/workspace picker 或真实公开 channel；pinned-cache upgrade/failed-upgrade recovery、final release trust 与 production Helper/Level 2 仍开放。
