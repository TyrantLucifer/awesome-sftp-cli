...
时序稳定性增量：SHA `60255dc8dc5e4732ae0fe55ac11ca860390bc81c`、tree `27fe962f3c712eae16e061a06d8460542eaaedf5` 为 Helper client/process/stderr 终态、tail 两次轮询和 Stage 2 PTY delete 选中态建立显式同步屏障，保留全部原 timeout/断言。push `29659577751`/PR `29659579470` 分类出 unchanged local-command 20 ms start miss，并暴露 introduced immediately-exiting exact-stderr child 的 stdout-close race。纠正 SHA `573aeefcaa488d73a7b5186a46ef03f87c1d8416` 恢复 resident child 与 Close barrier；PR `29660040623` 24/24，通过 22 jobs 与 quality/auth 的 push `29660039347` 又暴露 fixture 忽略的 32,768-byte short write。Exact-write SHA `60ff9080812e0e47f720d102671116a6557b9921` 以 `io.CopyN` 精确写满 65,536/65,537，并通过双 Go 完整本地门禁；其 push 通过 22 jobs 后仅在 Homebrew 更新遭遇外部 GitHub DNS 失败，PR 的 Helper 边界通过但发现 `cache:lru | normal` 在 loading 时也可见。待提交纠正使 copy/move/rename/delete 均等候 loaded `READ-ONLY | sort:name`，完整真实 PTY current race 20 轮与 exact oldstable 10 轮及最终双 Go 完整门禁通过。该批不生成 RC evidence，不改变 production Helper/Level 2 CLOSED 边界。
