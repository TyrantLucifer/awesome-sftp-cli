...
当前增量：foundation、doctor 与 support-bundle SHA `bfc59c8c0478a7043b8287f52d31dd3477b30554` 已交付 shared policy、公开只读 probes、public preview/create、manifest+八源、exact consent、deterministic bounds 和 owner-private durable publication。evidence SHA `496356908990d4bb5d9473acad754fd8ae7217ca` 的 push `29647975703` 与 PR `29647977377` 均为 22 success / 1 failure / 1 dependency-skipped；push 的 Ubuntu 22.04 stderr-overflow 与 PR 的 macOS ARM heartbeat 仅命中既有 Helper reader visibility timing，各自 same-SHA 对侧 companion 通过，新增包在失败 job 内也通过，未 rerun。hardening SHA `ba334b8d8968f5b09c91c0185996994b9a307ff4` 把 persistent component/event/error 与导出的 build/platform/doctor/diagnostic/Job/database/daemon 字段收紧到 semantic registry；safe-shaped 用户名、主机、私钥、Askpass、Kerberos、环境、命令参数、文件内容 canary 在 persistent JSON 和 expanded archive 中均为 0-hit。corrupt config/database/cache 与 unhealthy daemon 只导出 stable code，原 bytes 不变。focused normal/race/vet、exact current `make ci` 与 corrected-local-environment exact oldstable `make check` 通过。实际 auth-artifact/Helper 扫描、兼容矩阵、troubleshooting 与 finding ledger 仍开放。
