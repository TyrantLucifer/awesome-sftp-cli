...
**Timing-stability checkpoint**: A four-slice batch based on `c81887a8fcfacd488ff88c23ead46f34f2f972e5` adds explicit Helper client/process/stderr terminal joining, deterministic tail poll injection with the original 80 ms limit intact, and a stable Stage 2 PTY selection barrier before delete input. Current Helper/tail repetition, real PTY repetition, focused oldstable/vet/lint/docs, and complete exact current/oldstable local gates pass. This is pre-RC process/session and Hosted-fixture hardening; it does not create an RC record, close any release gate, or open production Helper/Level 2.
