...
**Timing-stability checkpoint**: SHA `60255dc8dc5e4732ae0fe55ac11ca860390bc81c`, tree `27fe962f3c712eae16e061a06d8460542eaaedf5`, adds explicit Helper client/process/stderr terminal joining, deterministic tail poll injection with the original 80 ms limit intact, and a stable Stage 2 PTY selection barrier before delete input. Helper/tail and real PTY repetition plus complete exact dual-Go local gates pass. Push `29659577751`/PR `29659579470` have same-SHA companion coverage for the unchanged local-command start-cancellation failure and exposed one introduced immediately-exiting exact-stderr fixture race. The correction restores the resident child and closes through the new terminal barrier before reading diagnostics; corrected current race/oldstable each pass 200 iterations and both full local gates pass. Corrected exact-SHA Hosted evidence remains open. This is pre-RC process/session and Hosted-fixture hardening; it does not create an RC record, close any release gate, or open production Helper/Level 2.
