...
- **Status**: In Progress — M6.1 complete; M6.2 migration, retention, Helper lifecycle, deterministic packaging/dependencies, native lifecycle, preview-archive picker, and pinned-cache recovery evidence active
- **Updated**: 2026-07-18
...
**Current checkpoint**: deterministic packaging/dependency closure, installed lifecycle, public Helper surface, release source, owner composition, four-platform native lifecycle, preview-archive SSH/TUI, and no-argument picker have exact-SHA evidence while production trust remains empty. Pinned-cache recovery SHA `925b22eb6a743816abcd11d1e552a7af1ce2992a` builds the lifecycle helper from frozen Stage 5 and current source, publishes an exact pinned-offline entry through production owners, and revalidates identity/bytes across controlled persisted failure, degraded default refusal, explicit resume to v4, frozen Stage 5 read-only rollback, and current restart. Exact current/oldstable local gates plus both Hosted quality jobs pass. The persisted failure is an honest boundary rehearsal using production attempt/verified-backup APIs, not a claim that a v4 SQL statement naturally failed. Public-channel execution, final license/notices, signing/notary, protected production Helper and Level 2 remain open.

**Latest increment**: implementation SHA `925b22eb6a743816abcd11d1e552a7af1ce2992a`, tree `540681e14deec18b6d86a3d74f738b129305db18`, passed the trusted-root lifecycle, exact current Go 1.26.5 `make ci`, and exact oldstable Go 1.25.12 `make check`. Push [29639963220](https://github.com/TyrantLucifer/awsome-sftp-cli/actions/runs/29639963220) finished 21 success / 2 failure / 1 dependency-skipped; PR [29639964534](https://github.com/TyrantLucifer/awsome-sftp-cli/actions/runs/29639964534) finished 22 success / 1 failure / 1 dependency-skipped. Both quality/auth paths and all new integration packages passed. Both ARM macOS failures were the existing Helper heartbeat race; push Intel macOS hit an existing cancellation-deadline race while the PR Intel companion passed. No rerun or weakening occurred. This closes the pinned-cache recovery rehearsal, not the real public channel or final release trust.
