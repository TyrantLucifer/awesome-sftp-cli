...
**Current checkpoint**: foundation SHA `ae172d9bb419bc6b0b05cf3ef3467258496d463f` supplies the shared sensitivity/export policy, fixed safe doctor report, and deterministic support-bundle core. Doctor implementation SHA `7aa6f20d1cada28064cfd4f169442ef219a9b1a8`, tree `df6450899ae9316600d38f72f426f3a4c308ce91`, now exposes the public command with help/man/static bash-zsh-fish completion parity and composes real read-only config/path/socket/daemon/OpenSSH/known-host/database/cache/Helper/disk/optional-endpoint probes. It creates no application path, does no repair/migration/authentication/upload, opens only stopped sidecar-free SQLite in immutable mode, bounds `ssh -G -T` to 64 KiB per stream, rejects ambiguous security fields and unknown host-key policy, and treats configured proxies as explicitly not probed. Direct endpoint reachability is a five-second no-payload TCP connect. Focused normal/race/vet, isolated-HOME no-creation smoke, exact current Go 1.26.5 `make ci`, and exact oldstable Go 1.25.12 `make check` pass. Hosted exact-SHA evidence is pending. Private support-bundle no-replace publication/cleanup, corrupted-state source composition, complete secret matrix, compatibility matrix, and threat/finding ledgers remain open.
