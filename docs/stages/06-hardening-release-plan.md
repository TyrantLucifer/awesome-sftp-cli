...
- **Status**: In Progress — M6.1 complete; M6.2 migration, retention, Helper lifecycle, deterministic packaging/dependencies, native lifecycle, and preview-archive first-run picker evidence active
- **Updated**: 2026-07-18
...
**Current checkpoint**: deterministic packaging `958dec07addbe12c0c02404af4baf90558286909` and dependency/SBOM `99db0e822b80d01519191e9bd13a894794c018c2` each passed exact-SHA push/PR Hosted 24/24. Installed lifecycle evidence SHA `d625a677f59e6e21d55516d2533db34c43d516fa` push passed 24/24 including exact v3→v4/rollback/uninstall; Homebrew renderer and public Helper status/install/upgrade/disable/remove contracts have exact-SHA evidence and keep arbitrary material/path input closed. Release source `53fceb0bf672607ba52d47550a65b095f3e0c76e` admits metadata before archive bytes and streams exact USTAR final candidates only after consent/probe/high-water; owner composition `9ee5afaa3290ccd2d6c00b0aee58563caae73844` adds private alias registry, restricted daemon RPC, durable removal recovery, fresh SFTP/OpenSSH and exact 0700 MKDIR while production trust remains empty. Native lifecycle SHA `99905179e1eac23819a8b7f25edffc07c29cf653` runs fresh owner-private binary/man/bash-zsh-fish install, version identity, daemon/Job start/status/stop, exact owned-file uninstall and SQLite retention on Ubuntu 22.04/24.04 and macOS ARM/Intel; PR passed the step on all four, push on two, and three earlier Helper timing failures each have an exact same-SHA successful companion. Preview-archive explicit SSH/TUI and no-argument host/workspace picker now have fresh-user Hosted execution. Pinned-cache/channel execution, final license/notices, signing/notary, protected production Helper and Level 2 remain open.

**Latest increment**: no-argument picker SHA `fbddcaa0208d2fca5c285162c5c9ffbf7fa8d318` starts the canonical archive binary with no arguments in a fresh HOME, renders SSH aliases, selects an isolated chroot host and proves its marker, then saves and reopens a two-remote workspace through a second no-argument launch. Both dependent auth jobs first verified the same-run SHA-bound bundle, then passed OpenSSH/TUI and Kerberos. PR [29638718437](https://github.com/TyrantLucifer/awsome-sftp-cli/actions/runs/29638718437) passed 24/24; push [29638717696](https://github.com/TyrantLucifer/awsome-sftp-cli/actions/runs/29638717696) passed quality/auth and only hit the existing oldstable macOS Intel Helper heartbeat-visibility fixture, with a successful exact-SHA PR companion and no rerun. This closes the preview-archive no-configuration host/workspace picker rehearsal, not a real public channel; pinned-cache upgrade/failed-upgrade recovery and final release trust remain open.
