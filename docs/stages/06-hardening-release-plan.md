...
**Timing-stability checkpoint**: SHA `60255dc8dc5e4732ae0fe55ac11ca860390bc81c`, tree `27fe962f3c712eae16e061a06d8460542eaaedf5`, adds explicit Helper client/process/stderr terminal joining, deterministic tail poll injection with the original 80 ms limit intact, and a stable Stage 2 PTY selection barrier before delete input. Push `29659577751`/PR `29659579470` classified one unchanged local-command start miss and exposed the introduced immediately-exiting exact-stderr fixture race. Correction SHA `573aeefcaa488d73a7b5186a46ef03f87c1d8416` restored the resident child and Close barrier; PR `29660040623` passed 24/24, while push `29660039347` passed 22 jobs plus quality/auth and exposed the fixture's ignored 32,768-byte short write. The final correction uses `io.CopyN` for exact 65,536/65,537 boundary writes; current race and oldstable each pass both cases 200 times and complete exact dual-Go local gates pass. Final corrected exact-SHA Hosted evidence remains open. This is pre-RC process/session and Hosted-fixture hardening; it does not create an RC record, close any release gate, or open production Helper/Level 2.
