...
**Current checkpoint**: deterministic packaging `958dec07addbe12c0c02404af4baf90558286909` and dependency/SBOM `99db0e822b80d01519191e9bd13a894794c018c2` each passed exact-SHA push/PR Hosted 24/24. Installed lifecycle fix `b7a740010c7e7188f9514e6676a0324aa34433a2` uses the prepared owner-private root, enforces it in docscheck, and passes the exact clean full lifecycle; replacement Hosted remains pending. The next four-slice batch exposes public Helper install/upgrade syntax, literal stable-home consent, pre-runtime rejection of arbitrary artifact/manifest/path/serve inputs, stable human/JSON closed-distribution errors, and help/man/completion parity. Current full `make ci` and exact oldstable full `make check` pass; commit, exact clean CLI and Hosted evidence remain. Native/channel, final license/notices, signing/notary, real Helper composition and Level 2 remain open.
