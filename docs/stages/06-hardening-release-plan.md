...
**Current checkpoint**: foundation SHA `ae172d9bb419bc6b0b05cf3ef3467258496d463f` supplies the shared sensitivity/export policy, fixed safe doctor report, and deterministic support-bundle core. Doctor implementation SHA `7aa6f20d1cada28064cfd4f169442ef219a9b1a8` exposes the public read-only command; docs SHA `d345c362ad8b6ea8c67b09a4e351ad27e929965a` has complete same-SHA Hosted classification, with only known Helper heartbeat/stderr reader visibility timing failures and no diagnostic regression. Support-bundle implementation SHA `bfc59c8c0478a7043b8287f52d31dd3477b30554`, tree `d519c6ae90d486733906c8125e5f80ecdb1e9c9e`, adds public preview/create, help/man/static bash-zsh-fish parity, real eight-source composition, post-preview drift refusal, owner-private 0600 no-replace/no-follow publication, file/parent sync, and cancellation/failure cleanup. Raw messages, IDs, paths, argv, locations, errors, credentials, prompts, and content are omitted or redacted; local daemon reads are bounded and absence/corruption degrades to safe summaries without starting it. Focused normal/race/vet/lint, isolated-HOME no-creation preview, exact current Go 1.26.5 `make ci`, and exact oldstable Go 1.25.12 `make check` pass. Exact-SHA Hosted evidence for this batch remains pending. The wider corrupt-state/secret matrix, compatibility matrix, and threat/finding ledgers remain open.
