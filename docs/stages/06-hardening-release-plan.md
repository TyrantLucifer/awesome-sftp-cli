...
**Current checkpoint**: REL-009 implementation SHA `19e8ab73d5f90a51815634b9b36112f86711f372`, tree `ceb8e749ac11c873b703d3bee3f3168cb9f5fce2`, has four executable documentation contracts and a linked install → first run → operations/recovery → upgrade/rollback → uninstall path. Exact command spellings, SSH/Kerberos prerequisites, default keymap/workspaces, transfer safety, Helper/direct fallback, doctor/support-bundle triage, durable Job recovery, and release withdrawal are documented. Exact current Go 1.26.5 `make ci`, exact oldstable Go 1.25.12 `make check`, focused docscheck, and public help/man/completion parity pass. Hosted push/PR `29656202301`/`29656203801` passed both quality/auth jobs; residual same-SHA-classified failures are existing OpenSSH-expansion and Helper-process visibility timing. The feature remains `In Progress` until an independent new-user exercise passes on the exact RC. No RC is frozen and no release/channel action is authorized.
