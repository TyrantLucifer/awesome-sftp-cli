...
- Ubuntu 22.04/24.04 amd64 runner 使用隔离 XDG 配置和测试 desktop handler 验证 xdg-open 的 MIME 分派；Linux arm64 原生安装/冒烟是 Stage 6 正式支持门禁。
- 两个平台实际启动 Vim，脚本化修改缓存文件并退出，随后验证变更检测、远端并发修改冲突与回传 Job。
- Stage 1 在 macOS Terminal/iTerm2、Linux 终端、tmux 和 screen 记录 resize、`SIGTSTP`/`SIGCONT`、Vim/shell 返回，以及正常退出、signal、panic 后备用屏幕/光标/输入模式恢复；发布候选复跑并记录到版本验证文档。
...
Stage 1 的结构性门禁使用纯 renderer fake 与 benchmark，证明构造 cell/分配只随可见窗口和有界 overscan 增长，不构造 50,000 个 UI 节点。下面的首屏时延、RSS、连续滚动/过滤与真实 provider 曲线是 Stage 5 的最终生产规模门禁；两者不能互相替代。
...
Stage 5 canonical gate 按 [ADR-0017](../architecture/adr/0017-stage5-unified-routing-direct-transfer-and-resource-budgets.md) 分解而不伪造物理吞吐证据：`TestWorkerHundredGiBSparseFileUsesSizeIndependentBoundedCheckpoint` 与 synthetic 对照证明真实 100 GiB metadata、64-bit offset、固定 buffer/checkpoint、cancel/part/final 边界；`TestWorkerHundredGiBContractLifecycleUsesSameCheckpointHashAndRateStateMachine` 在同一生产 Worker/Journal/Scheduler 上以多 quantum sparse-shaped 内容证明 pause、持久 checksum、重启实例续传、限速、完整 SHA-256 和 commit。Stage 6 nightly/release 仍必须把这些维度组合成一次完整物理 100 GiB LocalFS/SFTP 长跑并记录环境、用时和峰值资源。
