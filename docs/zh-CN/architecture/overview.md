...
这些约束要求 AMSFTP 把终端交互与持久工作分开，并明确两者之间的边界。
...
AMSFTP 只发布一个可执行文件，其中包含几个边界明确的角色。TUI 和 CLI 是短生命
周期客户端；本地 daemon 持有连接和长期任务；受限的内部角色处理认证和可选远端
集成，但不会成为独立服务。
...
因此，关闭 TUI 不会顺带取消传输。暂停、继续和取消仍是明确的 Job 操作。恢复也
只有一个责任主体：daemon 重启后会比较记录的进度和当前两端状态，再决定能否
继续。
...
**Location** 由这个 Endpoint 和绝对路径组成。因此，即使路径文本相同，只要
Endpoint 不同，就表示两个不同对象。
...
代码中定义了可选一次性远端 Helper 和远端直传的边界，但当前公开构建没有开启这
两条路径，日常浏览和传输也不依赖它们。增强路径不可用时，路由仍使用标准 SFTP
或本地中继。
...
流式复制先写入 Job 专属临时目标。冻结计划分别表达协议确认、目标 SHA-256 校验及
文件同步策略。普通复制默认要求写入确认、精确字节数、成功关闭与发布，不回读目标
也不强制同步。未完成内容不会以预期最终文件的身份出现。

协调器分别管理数据包、请求窗口和检查点预算。新复制计划最多传输 64 MiB，或在五秒后的下一个
数据包边界结束一个周期。journal 保存候选偏移与源摘要状态；带版本的完成证据另行
记录实际文件同步与目标内容证明。SQLite 原有的 `verified_offset` 列继续保存偏移，
不代表独立计算过目标摘要。旧计划保留同步检查点。恢复时即使弱元数据匹配，也会验证
候选内容；额外尾部经过证明才能回退。证据缺失或变化时报告冲突，不做无依据的截断
或追加。

新目录复制在最多八个文件事务之间共享 128 个数据请求槽。窗口随文件大小缩小，中继
两条链路分别计入预算，恢复验证也必须先预留预算再打开流。资源准入计算每文件缓冲、
数据包 worker 和传输窗口；父 journal 的更新仍然串行。检查点只保留有界的未完成子项，
重启后通过内容检查重建已完成条目。

严格 SHA-256 模式保留发布前后的回读。协议模式信任服务端成功的写入、关闭及发布确认，
不把元数据当作校验和的替代品。发布响应丢失仍需完整内容证明。标准 SFTP 不提供可靠的
内容版本 CAS；回读不能阻止第三方之后的修改，回读与 SSH 传输完整性也不能替代文件同步。

移动是在“复制并提交”之后，再单独检查并删除来源。目标验证和提交之前，来源不会
被删除。如果目标发布成功、但无法证明来源删除也成功，AMSFTP 会保留来源，并报告
尚未完成的部分。
...
状态格式通过受检查的迁移向前演进。迁移会先验证现有状态并保留恢复所需材料，再
发布新状态。如果结果不确定，启动会进入只读或以稳定诊断停止。删除数据库不是自动
修复策略，旧二进制也不能盲目写入较新的状态格式。
...
或报告稳定的资源错误；不会为了维持性能而牺牲完整性，或暗中开启权限更高的路由。
...
在 Linux 上运行 `make bench-transfer`，可使用真实 Worker、SQLite journal 和 SFTP
Provider，与系统原生客户端进行可复现的协议对照。该按需运行的测试需要 Python 3
和 OpenSSH 客户端、SFTP server，覆盖 0/20/80/200 ms 响应延迟、上传、下载、中继、
小文件目录，以及不限速和非零高限速。它记录耗时与数据字节数，可用
`AMSFTP_PERFORMANCE_OUT` 指定产物目录。原生上传分别记录普通复制，以及 fsync 加
两遍回读的耗时；后者使用独立客户端会话和本地回读文件。中继对照先用原生 get 暂存，
再用 put 上传。这些对照策略与 Worker 的执行路径并不完全相同。延迟代理不包含
SSH 加密、TCP 拥塞或真实链路带宽；常规原生集成测试覆盖 SSH 边界。Nightly CI
除确定性的窗口、检查点和恢复测试外，也运行这组对照。

`make bench-transfer-ssh` 用真实本机 TCP、系统 OpenSSH 加密及私有 sshd fixture 补充
代理测试。它对 256 MiB 上传、下载各重复三次，并交替执行顺序。SQLite 在计时前打开，
与已运行的 daemon 一致；计时包含连接、计划、Job 创建及执行，也单独报告 Worker
耗时。最终文件的摘要在计时区域外检查。默认复制及完成时同步分别与同语义原生操作
对照，SHA-256 模式则以普通复制为参照报告额外开销。原生客户端直接写入最终名称，
AMSFTP 的计时则包含临时文件发布和 Job 记录成本。测试不会修改用户 SSH 配置。
`AMSFTP_PERFORMANCE_OPENSSH_DIR` 可选择包含 `ssh` 和 `sftp` 的可信目录，让双方使用
相同版本。结果不代表某个用户实际广域网或存储系统的吞吐能力。
