...
流式复制先写入 Job 专属的临时目标。只有对应数据跨过持久化边界后，进度才会持久保存；
内存中的实时进度另行报告传输流正在消耗的字节数。
随后 AMSFTP 验证临时结果并发布最终名称。未完成的内容不会以预期最终文件的身份
出现。

传输协调器分别管理数据包、在途请求和检查点预算。Provider 管理有界的协议并发，
并报告已确认的写入；只有同步并检查后的数据才能推进持久记录。冻结的流策略将一个
周期限制为最多 64 MiB，或一秒后的下一个数据包边界。恢复时会先证明未记录尾部的
内容，再将它回退。目录 worker 最多持久保存两个独立子事务，并串行更新父检查点。
资源准入同时计算传输窗口、校验读取和有界目录并发的成本。

校验与发布仍各自承担独立职责。发布响应丢失时，同一次提交中成功取得的校验和证明
可以复用。标准 SFTP 不提供足够可靠的对象或版本身份，无法让 size/mtime 观察代替
完整的发布后校验；临时目标和发布后目标因此都保留内容检查。
