...
从已有的 SSH 账号开始，按本指南完成一次文件传输。认证和主机密钥决定
由系统 OpenSSH 处理，AMSFTP 不另外维护凭据。
...
别名可以继续使用系统中已有的 OpenSSH 配置，包括 Agent、ProxyJump、
...
请在系统客户端中核对新增或发生变化的主机密钥，并完成交互认证。AMSFTP 不会静默
...
`doctor` 会对本地安装、运行目录、daemon、OpenSSH 策略和所选端点执行只读检查，
不会改动远端文件。如果使用代理，端点可能显示为没有直接探测；这不表示认证失败。

如果 SSH 别名使用 Kerberos，请先用常用的系统工具检查或续期票据。
...
不带位置启动时，选择器会列出保存过的工作区、`local` 和 OpenSSH 配置中的明确
别名。带通配符的 `Host` 模板不会作为可选服务器列出。
...
`y` 不会读取或上传文件。按下 `p` 并创建 Job 后，传输才会开始。所有写入得到确认、
文件成功关闭并完成发布后，目标才会以最终名称出现。可选的 SHA-256 校验与同步策略
见[传输指南](transfers.md)。

按 `q` 可以退出 TUI，后台 Job 会继续运行。重新打开 AMSFTP，或运行
`amsftp job list` 即可查看这些 Job。
...
重复复制步骤。如果认证需要交互，AMSFTP 会使用 OpenSSH 流程，而不会要求保存
...
如果保存的远端目录已经不存在，AMSFTP 会尝试最近的可访问上级目录，并报告实际
打开的位置。
...
- [故障排查](../help/troubleshooting.md)按现象组织处理方法。
