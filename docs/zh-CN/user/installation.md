...
需要由包管理器维护时使用 Homebrew；需要用户自有的安装前缀且不使用包管理器时，
使用独立安装器。

> [!WARNING]
> macOS 公开构建既未签名，也未公证。请只从项目 Release 页面下载。不要为了
> 运行未经验证的程序而关闭系统级安全保护。
...
官方安装器默认使用 `$HOME/.local`，不会调用 `sudo`：
...
安装器会下载适合当前平台的归档并校验 SHA-256。随后检查安装路径和数据路径的
所有者与权限，再原子替换程序。它也会安装 man 手册并生成 shell 补全。
...
如有需要，请把这行加入当前 shell 的启动文件。

### 如果安装器要求使用托管根目录

如果路径的所有者、权限、ACL 或符号链接不安全，AMSFTP 会拒绝把私有状态放在该
路径下。如果普通主目录布局未通过这些检查，安装器会寻找
`/var/lib/amsftp-users/<你的-uid>` 这个私有根目录。

如果目录不存在，安装器会在不改变目标的情况下退出，并打印准确的一次性管理员
命令。请让管理员执行这些命令，再重新运行安装器。不要通过放宽主目录或 AMSFTP
数据目录权限来绕过检查。
...
运行 `install.sh --help` 可以列出当前的 `--prefix`、`--root`、`--version`
和 daemon 启动选项。
...
开发构建用于本地测试，不属于自动发行升级渠道。

## 升级

Homebrew 和官方独立安装都支持以下升级命令：

```sh
amsftp upgrade
```

AMSFTP 会先检查是否有新版本，再停止任何东西。如果某个 Job 正在传输数据，
AMSFTP 会拒绝升级，而不是在没有提示的情况下中断 Job。请先完成或暂停重要工作，
再重试。

升级命令只会恢复升级前原本就在运行的 daemon，还会检查新程序与 daemon 的版本
是否一致。如果它报告升级只完成了一部分，不要删除 socket 或状态数据库。请运行：
...
独立安装用户可以重新运行已经验证的安装器。如果原先确实存在一个旧版本，安装器会
...
再使用当前程序的只读 `doctor` 结果确定安全恢复路径。不要删除控制 socket，也不要
复制或替换正在使用的 SQLite 数据库来强行回滚。
...
删除这些路径前，请备份仍可能需要的内容。不要删除整个 home、XDG、`/tmp` 或
