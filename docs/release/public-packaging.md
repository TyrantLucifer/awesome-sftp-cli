...
The repository intentionally does not contain a placeholder project license or credential. Until the owner supplies both license files, creates the tap repository, configures both protected write tokens, and pushes the first strict tag, this automation fails closed and no quick-install channel exists. The workflow publishes an unsigned public preview, not a signed/notarized final release or public 1.0. Production Helper and Level 2 remain CLOSED.
