...
There is one bounded crash-recovery case: when the canonical path is still a validated private Unix Socket and the private Unix Socket has no live instance lock, `daemon status` reports stopped without removing anything and `daemon start` may invoke the starter. Only the new daemon process after it acquires the matching instance lock revalidates and removes that exact residual Socket before binding. A held or uncertain lock, regular file, symbolic link, unsafe Socket, authentication failure, or protocol failure remains fail-closed and must not start or delete anything.
