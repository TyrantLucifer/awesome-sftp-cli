...
- **Current implementation checkpoint**: the shared `MutableProvider` contract now runs against Fake, LocalFS and protocol SFTP. LocalFS mutations use Go's rooted filesystem handle so parent symlinks cannot escape the configured root; final no-replace publication uses an atomic hard-link appearance followed by separately observable part cleanup. SFTP exposes `write` only when the current server session advertises both `fsync@openssh.com` and `hardlink@openssh.com`; weak servers retain read-only capability instead of receiving an unsafe fallback. Frozen `FileRef`/Intent/Plan tests cover source identity, capability revisions, local/SFTP relay routes, conflict policy and caller mutation after freeze. The complete immutable Plan, including its original requested name, is now persisted and cross-checked against indexed operation columns before daemon restart execution. The bounded single-file worker writes only the same-directory Job part, handles partial writes, syncs and persists SHA-256 state at each safe offset, rereads the part for full checksum verification, rechecks the final at commit, and proves postconditions after an indeterminate rename response. Pause/cancel close and restat the part before their final checkpoint so close-time filesystem metadata cannot make a safe checkpoint stale. A bounded daemon-owned manager reloads queued plans from SQLite, owns worker contexts independently from clients, transitions ordered Job events, and durably pauses/resumes a gated real LocalFS transfer. Local commit-time race tests cover ask, skip, overwrite and auto-rename while asserting the final is absent at the verified-part boundary.
- **Focused evidence**: `go test -race -count=1 ./internal/provider/... ./internal/transfer ./internal/state/jobstore` and `make lint` pass. The guarded real-OpenSSH fixture now performs both local→SFTP and SFTP→local worker copies and requires the real server's durable write capability; it remains pending Hosted execution with `AMSFTP_REAL_SSHD=1`.
- **Open gate**: daemon-owned scheduling/control and endpoint lifetime, IPC/TUI intent/event replay, retry/auth state mapping, broader fault/crash evidence and the user-visible MVP remain required before M2.2 can be marked complete.
