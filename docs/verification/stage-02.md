...
- **Current action**: integrate the upgrade coordinator, generalize backup verification beyond the fixed V1 head, and close sidecar/bootstrap crash plus native cross-process recovery probes.
...
- **Last green command**: current `make check`, exact Go 1.25.12 `make check`, focused current race, `make lint` with zero issues, and four-target `CGO_ENABLED=0` product builds all passed on the WAL candidate.
