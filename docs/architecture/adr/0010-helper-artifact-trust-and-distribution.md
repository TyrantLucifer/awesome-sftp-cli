...
8. final consent后才创建/复核exact`0700`目录；在verified sibling用`O_WRONLY|O_CREATE|O_EXCL`打开temp。因 ADR-0011 的 pkg/sftp v1.13.11 fork 基线仍按 SFTP v3 OPEN 语义且不带permission attrs，首个content byte前须handle `Chmod(0600)`，再分别handle stat/path lstat确认owner、regular、mode0600、size0、非symlink；SFTP v3无stable file-id，不声称二者证明同一object。entropy/O_EXCL/attrs失败不写byte。
