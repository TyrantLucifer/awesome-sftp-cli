...
daemon 写 owner-only、有大小与保留上限的结构化日志。Jobs 抽屉和 CLI 暴露稳定状态、阶段、计数器、等待原因与控制动作。传输速率使用每个可见 Job 最多 16 个样本形成四秒滚动窗口；relay 阶段耗时作为有界、脱敏的 checkpoint/JobView 字段供现场诊断。`doctor` 只读检查配置、运行目录、socket、daemon、OpenSSH、known-host policy、数据库、缓存、Helper 和磁盘空间。
