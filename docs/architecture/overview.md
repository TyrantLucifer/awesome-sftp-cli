...
本文主体描述 1.0 目标架构，不代表所有运行时组件已经存在。Stage 0 foundation、Stage 1 Read-only Explorer、Stage 2 durable transfers，以及 Stage 3 Preview/Edit/Cache 日常切片已在 Stage 3 候选分支实现：除了只读浏览、工作区和 part/verify/commit Job，现在还包含 Preview/Jobs/Log drawer、有界文本/代码/JSON/二进制/图片预览、owner-only LRU/cache lease、`e`/`o` durable edit recovery、唯一 Stage 2 sync-back Job 路径、显式 `!`/`gs` shell surface 与 terminal handoff。精确候选身份和平台证据以 [Stage 3 verification](../verification/stage-03.md) 为准。remote Helper/search/watch/tail、cross-host direct transfer、大规模加固与发行就绪仍属于 Stage 4–6；整个产品的 production/release readiness 只由 Stage 6 门禁判定。
