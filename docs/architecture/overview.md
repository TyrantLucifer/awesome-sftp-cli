...
- [ADR-0016：Stage 4 Search 与可选 Helper 运行时契约](adr/0016-stage4-search-helper-runtime-contracts.md)
...
任何 application-managed install-tree bytes 都服从两次确认与固定顺序：当前 policy 的 Ed25519/key/artifact 撤销、签名目标/协议/`min_client`、release floor 与兼容检查后，先把exact raw manifest/signature持久化但不enable，再展示 preliminary mapping/probe consent；取消时必须产生零次 probe。获准后才做 fresh root-owned utility/observed non-root numeric uid/SFTP-exec namespace probe、目标匹配与 Endpoint `(version, hash)` high-water 检查、本地产物 size/hash 校验，并派生 strict-safe canonical home、1000-byte remote path ceiling 和完整安装祖先计划；再展示 final actual-plan consent，取消时 application-managed install tree 必须保持零 create/content。最终获准后才上传，且由客户端回读复算 hash。publication要求OpenSSH `hardlink@openssh.com` target-exists-fails语义；扩展缺失时fail closed，不以可能覆盖的SFTP v3 rename或delete-first降级。probe/hash/path string 只在受信跨 session shared-home mapping 下成立，不证明同 node/mount/object。每次enable从canonical raw bytes重跑当前policy、fresh binding/attrs/hash和exact handshake；disable/remove保留high-water。GitHub attestation 与 TLS 只能补充、不能替代该离线信任根；完整生命周期见 [ADR-0010](adr/0010-helper-artifact-trust-and-distribution.md) 与 [ADR-0016](adr/0016-stage4-search-helper-runtime-contracts.md)。

Stage 4 search以相同identity contract在Level 0 Provider与Level 1 Helper之间路由。Helper request开始失败可让新请求选择Level 0；一旦流开始，崩溃只把该请求标为partial，不能把两次transport结果拼成同一complete snapshot。same-host copy同样只在Plan阶段选择：Helper只创建冻结的Stage 2 part，Worker继续拥有checksum、conflict、commit、checkpoint与restart。
...
| 等级 | 能力 |
| --- | --- |
| Level 0：标准 SFTP | 浏览、基础文件操作、流式读写、断点续传、本机有界中继、受限递归文件名搜索与显式确认的慢速内容搜索 |
...
- 分段检查点、临时目标与验证结果
- 缓存索引、租约、配额和最后访问时间
- 编辑会话状态、决策输入、dirty materialization 恢复引用与 sync-back Job 绑定
- RPC 去重窗口和可恢复事件游标
...
默认缓存是有配额的 LRU；workspace 可以选择 ephemeral 或明确 `pinned_offline`。默认全局 2 GiB/4096 managed objects（entries+materializations），每 workspace 非共享份额 1 GiB，单次 reconciliation 最多考察 256 个候选。内容与元数据分离：

- 内容按可验证身份去重。
- 缓存条目记录远端版本指纹、fresh/stale/unknown 和来源 Location；同路径不等于同一内容。
- preview/edit/open/upload 期间持有 reference/lease，dirty、pinned、active/uncertain lease、Stage 2 part 和共享引用都是 eviction 保护集。heartbeat 默认 30 秒，expiry 2 分钟，立即返回 opener 的 grace 默认 15 分钟。
- 守护进程重启后，普通 lease 只在 PID+birth identity 证明 owner 已死且 expiry/grace 已过时回收；无法证明时保留并诊断。特例是可重建的 Preview handoff：若 exact process identity 已死且恰有一条匹配 reference，可在 expiry 前立即回收；editor/opener/upload 不走此快速路径。
- 发布 entry 或 materialization 前的 serial admission 将 incoming object 纳入 bytes+object count，在最多 4 个批次内逐次重读/重规划；无法在不删除受保护内容的情况下纳入时 typed `resource_exhausted`，不先超配额再事后清理。Provider short read/oversize 在 staging 中就按 expected size/hash 拒绝。
- startup 用 256 条/page 的 keyset 分页遍历，默认最多 64 批（单次 16,384 个 candidate）；只回收可证明已死且恰有一条匹配 reference 的 Preview handoff，edit/open/upload 引用和 ambiguous/uncertain owner 继续保留。
- disconnected `pinned_offline` 只复用同 workspace+Location 的唯一 strong historical fingerprint，重验 entry ID、manifest/catalog、blob SHA/size后标记 freshness `unknown`并取得新 lease；多版本歧义或任一 identity 不可证明即 fail closed。重连后必须从 Provider 重新验证，离线副本不能直接授权回传。
- 缓存文件使用当前用户私有权限，不承诺替代凭据隔离或磁盘加密。

cache filesystem manifest 不包含完整的 policy/dirty/reference/lease 元数据。所以 catalog/manifest 损坏或两侧不一致时，实现进入 fail-closed diagnostic-preservation：保留未知 bytes，拒绝 quota/clear/materialize，不从现有 manifest 猜测安全元数据或声称已自动 rebuild。此时 Stage 1 browsing 和已有 Stage 2 Jobs 仍可用。精确合约见 [ADR-0014](adr/0014-stage3-cache-preview-and-edit-session-contracts.md)。
...
1. `K` 把当前 FileRef、Endpoint session/generation、fingerprint、request ID、mode/offset 和 UI generation 冻结为 Preview identity；cursor/refresh/Endpoint/mode 变化先 cancel 旧请求，结果 identity 不全等则丢弃。
2. 守护进程按 64 KiB head/tail/range/continue 计划读取，并在每次合并后将 retained window 限制为 512 KiB；short read 按冻结 source 已变化处理，不把旧新 bytes 混合。
3. built-in renderer 在 512 KiB input/output、256 KiB JSON、64 depth、10,000 lines/4,096 style spans 内产生 text/code/JSON/metadata/binary 结果；control/ANSI/NUL/invalid UTF-8 会清洗或回退 hex，partial/range/discarded 显式显示。metadata 无需读 content 即可展示经单行/每字段有界清洗的 Endpoint、canonical path、kind、size/mtime/mode、symlink target、fingerprint/hash/file/version ID、type/range/status。
4. image 只在 200 ms/256-byte active probe 精确确认 Kitty/iTerm2/Sixel 后输出，并受 4 MiB payload、6 MiB output、1,000,000 encoded pixels 限制；否则保留 built-in metadata/binary fallback。
5. 仅 image/unknown binary 可按有序 rule 尝试 external previewer；先取得 complete verified materialization+lease，并在 max-input/timeout/identity/4 KiB redacted diagnostic 边界内 direct exec。任何失败都只回退 built-in Preview。
...
1. 守护进程将 complete verified blob 复制为独立 materialization，持久 baseline/session/reference/lease，再把可验证 local path 交给 client。blob 从不原地修改。在线 baseline 还必须在 expected fingerprint 下对 Provider stream 计算全文 SHA-256/size，并与 materialization 一致。
2. client 按 explicit config→`VISUAL`→`EDITOR`→`nvim`→`vim`→`vi` 解析 editor；opener 默认是 macOS `/usr/bin/open` 或 Ubuntu `/usr/bin/xdg-open`。受限 env lexer 不做 expansion/operator，PATH 只用于发现，executable 与独立末参数文件路径在确认后 direct exec。
3. external handoff 恢复终端后，客户端重做 image capability probe。editor 无论正常/非零/signal 都观察 local，opener 则保留 15 分钟 grace 并等待用户显式“检查更改”；两者都不把退出或 mtime 当成上传授权。
4. baseline/local-current/remote-current 决定无改动、local-only、remote-only、both 或 unknown。remote-current 对普通文件同样在 2 GiB cap 内 stream-hash，因此同 stat 但不同 bytes 不会被当作 unchanged。local-only 需显式上传确认；remote-only 刷新；both/unknown 进入 conflict。
5. conflict viewer 重验 local SHA/no-follow 和已观察 remote fingerprint，每输入最多 32 KiB、512 行，输出最多 64 KiB。用户可 inspect、save-as、skip/retain 或显式 overwrite；决定绑定原 edit session 并持久审计。
6. 确认后只能原子冻结 edit-session→Stage 2 Plan/Job，执行 `.part`→verify→commit 并在开始/提交前重验 expected destination fingerprint+content SHA。pre-publish content hash 不等时保留 part 并进入 conflict。失败、client/daemon restart 或再次变化都保留 dirty materialization，`E` 从持久记录恢复。

### 10.4 显式 command 与 shell

- `!` 是用户确认的单行命令，最多 32 KiB；local 用 absolute shell、`-c`、独立用户原文和 process cwd，stdout/stderr 各1 MiB ring 并继续 drain 超额 bytes。client 仅允许一个 active command，Normal `Esc` 取消，硬 timeout 为 15 分钟；并发第二个请求 bounded 拒绝。
- remote `!` 使用 ADR-0001 fresh `ssh -T`，关闭 forwarding/GSS delegation/ControlMaster；只在 root-owned utility 与 POSIX byte-quoted cwd bootstrap 通过、stdout byte-0 marker 到达后才写入用户命令。marker 失败时写入 0 command bytes；cancel/detach 显示 `remote effect unknown`。
