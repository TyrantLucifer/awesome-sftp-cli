...
host_key_failures_before="$(grep -c '"event":"rpc_request_failed"' "${daemon_log}" 2>/dev/null || true)"
...
host_key_failures_after="$(grep -c '"event":"rpc_request_failed"' "${daemon_log}" 2>/dev/null || true)"
if ! test "${host_key_failures_after}" -gt "${host_key_failures_before}"; then
  printf 'host-key case did not add a failed daemon request record\n' >&2
