...
platform | Linux arm64 | native-tested | release target pending final archive/channel evidence | Hosted ubuntu-24.04-arm complete native lifecycle and exact ARM64/linux/arm64 provenance; independent cross-build and reproducibility
...
direct-transfer | production Level 2 | untested | CLOSED; bounded relay remains default | real isolated network data-plane proof remains open`)
	...
	for _, required := range []EvidenceStatus{NativeTested, FixtureTested, BestEffort, Unsupported, Untested} {
		if !seen[required] {
