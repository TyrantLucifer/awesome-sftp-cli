...
func EnvironmentMatrix() []EnvironmentCase {
	return []EnvironmentCase{
		...
		{Area: "platform", Case: "Ubuntu 24.04 amd64", Status: NativeTested, Behavior: "supported", Evidence: "Hosted /usr/bin/ssh reports OpenSSH_9.6p1 Ubuntu-3ubuntu13.18; version-bound doctor, full authentication/support scan, ext4/XFS and current/oldstable gates"},
		{Area: "platform", Case: "Linux arm64", Status: NativeTested, Behavior: "release target pending final archive/channel evidence", Evidence: "Hosted ubuntu-24.04-arm complete native lifecycle and exact ARM64/linux/arm64 provenance; independent cross-build and reproducibility"},
		...
		{Area: "sftp", Case: "standards-compatible SFTP v3 without OpenSSH extensions", Status: FixtureTested, Behavior: "Level 0 browse and download remain available; unsafe publication fails without mutation", Evidence: "separate-process virtual server browse, stat, read, raw-byte and no-extension gates"},
		{Area: "sftp", Case: "other vendor servers", Status: BestEffort, Behavior: "standard SFTP baseline; extensions are capability-gated", Evidence: "native vendor matrix remains open"},
