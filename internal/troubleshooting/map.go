...
func Entries() []Entry {
	return []Entry{
		...
		{DomainError, string(domain.CodeIntegrityFailed), "Content, metadata, protocol, executable, or persistent-state integrity validation failed.", "Preserve the evidence and stop mutation of the affected object. If v0.1.0 reports `validate authentication helper` from a Homebrew install, stop the daemon with `amsftp daemon stop --confirm stop`, restart it once with `\"$(brew --cellar amsftp)/0.1.0/bin/amsftp\" daemon start`, and upgrade to the latest patch release; do not loosen modes, ACLs, Gatekeeper, or SSH policy."},
