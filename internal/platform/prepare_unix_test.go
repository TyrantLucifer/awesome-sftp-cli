...
func TestPreparePrivateDirectoryCreatesAndValidatesPrivateRoot(t *testing.T) {
	...
	if err := PreparePrivateDirectory(path, ValidateRuntime); err != nil {
		t.Fatal(err)
	}
	...
	if err := ValidatePrivateDirectory(path, ValidateRuntime); err != nil {
		t.Fatal(err)
	}
}
...
func TestPreparePrivateDirectoryRejectsSymlinkComponent(t *testing.T) {
	...
	if err := PreparePrivateDirectory(filepath.Join(link, "child"), ValidateRuntime); err == nil {
