...
AMSFTP 是一个 Vim 风格的双栏文件管理器，用于管理本地文件和 SFTP 服务器。
每一栏都可以打开本地目录或 `~/.ssh/config` 中的主机，无需离开终端工作区即可
浏览、预览、编辑、搜索和传输文件。

> [!WARNING]
> AMSFTP 目前是公开预览版，还不是 1.0。macOS 构建既未签名，也未公证。公开
> 版本支持标准 SFTP；远端加速和服务器间直传仍不可用。
...
- 把复制、移动、重命名和删除作为可恢复的后台 Job 执行，分别管理带宽、请求窗口和
  恢复预算；目录内的独立文件可以并发传输。
- 普通复制在服务端确认后发布完整文件；可选 SHA-256 校验和文件同步，移动与编辑
  回写保留更严格的保护。
- 预览文本和图片，在本地编辑或打开远端文件。
- 按文件名或内容搜索，并在搜索规模较大时明确提示限制。
- 使用 `doctor`、结构化诊断和需要明确同意才能创建的支持包排查问题。

AMSFTP 不会保存密码、私钥、Agent 内容、Kerberos 票据或认证回答。

## 系统要求

- macOS 15 或更高版本，或者 Ubuntu 22.04/24.04。其他 Linux 发行版仅提供
  尽力而为的支持。
- 系统自带的 `/usr/bin/ssh`。
...
安装器会在替换程序前校验发行包 checksum。如果主目录布局未通过所有者或权限
检查，安装器会打印准备私有托管安装根目录所需的准确命令。
...
先用 OpenSSH 完成第一次主机密钥确认和认证，再检查端点：
...
位置可以是本地绝对路径，也可以是
`<SSH-别名>:<远端绝对路径>`。不带位置运行 `amsftp` 会打开启动选择器。
使用 `amsftp --workspace <名称>` 可以重新打开保存过的工作区。
...
复制和剪切只记录选择。粘贴会先展示目标位置和需要确认的选项，再创建 Job。新内容
通过所选完成检查并提交后才会以最终文件出现；移动操作也会等到目标完整后再删除来源。
远端到远端复制通过本地 daemon 使用受限内存中继，不会把 SSH 凭据复制到任何一台
服务器。
