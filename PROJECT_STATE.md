...
Define the M1.4 CLI/Host/workspace contracts test-first: freeze zero/one/two Location precedence, selectable OpenSSH Host-alias discovery, and the versioned secret-free two-pane workspace schema before wiring persistence or picker UI.
...
- M1.1–M1.3 have local and Hosted implementation evidence. Include/Match and ControlMaster new/reuse coverage, workspace persistence, Host picker and recovery remain M1.4/final Stage 1 gate work and are not yet claimed. The current pkg/sftp public API completes a remote `ReadDirContext` before daemon pages are emitted; true source-streamed enumeration remains an explicit Stage 1 exit item, not a hidden M1.2 claim.
