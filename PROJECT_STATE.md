...
The approved Stage 0 baseline remains `d637474ac52ef2c5b9f78c9be663e52c6a9f441c`. M1.1 is complete at `8e649f534b500e494ec2984a763e4491711df5fe` with fully green Hosted run [29399674061](https://github.com/TyrantLucifer/awsome-sftp-cli/actions/runs/29399674061). M1.2 is complete at `28f8731604201763e48bf43c5a7f7e2a7014ca6c` with fully green Hosted run [29401801663](https://github.com/TyrantLucifer/awsome-sftp-cli/actions/runs/29401801663). M1.3 is complete at `7f0ea00981cecd5799b3c17ee56eff204cfd5a90`: the bounded Auth Broker and same-binary askpass/TUI flow pass focused race/vet, while Hosted run [29408865534](https://github.com/TyrantLucifer/awsome-sftp-cli/actions/runs/29408865534), job [87330882913](https://github.com/TyrantLucifer/awsome-sftp-cli/actions/runs/29408865534/job/87330882913), passed real key, agent, ProxyCommand, ProxyJump, password, wrong-password, cancel, MFA, host-key and MIT Kerberos/GSSAPI matrices with secret scans. M1.4 implementation and platform evidence are present through candidate `56e5f4f6c4579cbafa6e38a237720efc1686b098`: zero/one/two Location startup, Host/workspace picker, secret-free atomic workspace persistence, pane-local endpoint transactions, bounded diagnostic correlation, Vim counts/repeat and multiline preview are covered locally, while real kernel ACL/lock/hostile-UID security fixtures passed on all four native platforms in [run 29417470068](https://github.com/TyrantLucifer/awsome-sftp-cli/actions/runs/29417470068). All other independent jobs in that run passed, but the real recovery scenario still fails after a successful SSH reconnect because the invalid directory's `not_found` result does not advance to nearest-parent recovery. That issue has reached the three-attempt stop rule. Stage 1 also remains In Progress because exact `github.com/pkg/sftp v1.13.10` buffers a complete remote directory before returning from its only public context-aware directory API, so true source-incremental SFTP enumeration cannot be delivered without changing the approved dependency boundary.
