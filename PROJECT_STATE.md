...
The approved Stage 0 baseline remains `d637474ac52ef2c5b9f78c9be663e52c6a9f441c`. M1.1 is complete at `8e649f534b500e494ec2984a763e4491711df5fe` with fully green Hosted run [29399674061](https://github.com/TyrantLucifer/awsome-sftp-cli/actions/runs/29399674061). M1.2 is complete at `28f8731604201763e48bf43c5a7f7e2a7014ca6c` with fully green Hosted run [29401801663](https://github.com/TyrantLucifer/awsome-sftp-cli/actions/runs/29401801663). M1.3 is complete at `7f0ea00981cecd5799b3c17ee56eff204cfd5a90`: the bounded Auth Broker and same-binary askpass/TUI flow pass focused race/vet, while Hosted run [29408865534](https://github.com/TyrantLucifer/awsome-sftp-cli/actions/runs/29408865534), job [87330882913](https://github.com/TyrantLucifer/awsome-sftp-cli/actions/runs/29408865534/job/87330882913), passed real key, agent, ProxyCommand, ProxyJump, password, wrong-password, cancel, MFA, host-key and MIT Kerberos/GSSAPI matrices with secret scans. M1.4 implementation is present through candidate `4803d2789504d00566d51282b82c5d6fa5bf561a`: zero/one/two Location startup, Host/workspace picker, secret-free atomic workspace persistence, pane-local endpoint transactions, bounded diagnostic correlation, Vim counts/repeat and multiline preview are covered locally. Hosted [run 29416890911](https://github.com/TyrantLucifer/awsome-sftp-cli/actions/runs/29416890911) passed quality, native/oldstable, builds and reproducibility, and its auth job passed the full authentication matrix, but the real recovery scenario still fails after a successful SSH reconnect because the invalid directory's `not_found` result does not advance to nearest-parent recovery. That issue has reached the three-attempt stop rule. Stage 1 also remains In Progress because exact `github.com/pkg/sftp v1.13.10` buffers a complete remote directory before returning from its only public context-aware directory API, so true source-incremental SFTP enumeration cannot be delivered without changing the approved dependency boundary.
...
Choose the recovery/streaming resolution after the recorded stop: authorize a focused application recovery-state fix with deterministic orchestration tests, and approve either a narrowly maintained immutable `pkg/sftp` fork/replace exposing a directory cursor or an ADR-backed dependency/API change. Then re-run the exact Stage 1 gate without introducing a second SFTP protocol stack.
...
The active command/result ledger is [Stage 1 verification](docs/verification/stage-01.md); [Stage 0 verification](docs/verification/stage-00.md) remains the completed foundation handoff. Initial Stage 1 safety checks passed on 2026-07-15: clean branch `codex/stage1-read-only-explorer`, HEAD `d637474ac52ef2c5b9f78c9be663e52c6a9f441c`, tree `83a515607f44f7edb85f8103962b6d9d1173c02d`, and matching `origin/codex/stage1-read-only-explorer`. On the M1.4 implementation, local `make ci` and exact `GOTOOLCHAIN=go1.25.12 make check` pass; this includes full race, lint, docs, supply-chain, four fuzz smokes and four target builds. Hosted run [29416890911](https://github.com/TyrantLucifer/awsome-sftp-cli/actions/runs/29416890911) is deliberately recorded as failed, not green: every non-auth job passed, the auth matrix passed inside its job, and the recovery postcondition timed out after reconnect at `hosted-stage1-recovery.py:322` waiting for nearest-parent recovery.
...
- Final platform-security fixtures now cover real Darwin deny/direct/inherited ACLs, Linux access/default ACL xattrs, cross-process locking and a root-gated hostile-other-UID socket peer. Local race, exact Go 1.25.12, docs and actionlint checks pass; the new four-platform native CI step has not yet run on a pushed exact candidate, so DAEM-002/SEC-001 remain `Implemented` rather than `Verified`.
- M1.1–M1.3 have local and Hosted implementation evidence. M1.4 now has local evidence for CLI/workspace/picker/recovery and Hosted evidence is being finalized; Include/Match and ControlMaster new/reuse already pass the current authentication matrix. The exact `pkg/sftp v1.13.10` public API completes a remote `ReadDirContext` before daemon pages are emitted. Its `Walk` path delegates to the same complete-slice API, and v1.13.11 does not change that public behavior. True source-streamed enumeration therefore remains an explicit Stage 1 exit blocker, not a hidden M1.2/M1.4 claim; resolving it requires an approved fork/replace or ADR-backed dependency/API change.
