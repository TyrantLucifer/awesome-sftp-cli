...
**Hosted timing-stability increment**: Four related RED/GREEN slices at SHA `60255dc8dc5e4732ae0fe55ac11ca860390bc81c`, tree `27fe962f3c712eae16e061a06d8460542eaaedf5`, require explicit Helper terminal-state joining, deterministic tail polling, and a stable selected PTY target before delete input. GREEN joins the client reader, OpenSSH process, and stderr reader through `ProcessSession.Wait`; retains the tail fixture's 80 ms limit while injecting its two poll events; and requires the Stage 2 screen to show both the filename and a loaded status before `D`. Helper/tail repetition, real Stage 2 PTY, and exact dual-Go local gates pass. Push `29659577751` has same-SHA PR quality coverage for an unchanged 20 ms local-command start miss; PR `29659579470` exposed the introduced immediately-exiting exact-stderr child's stdout-close race. Correction SHA `573aeefcaa488d73a7b5186a46ef03f87c1d8416` restored the resident child and Close barrier; PR `29660040623` passed 24/24, while push `29660039347` passed quality/auth and 22 jobs before native Intel observed a 32,768-byte short write from the fixture's ignored single `os.Stderr.Write`. Exact-write SHA `60ff9080812e0e47f720d102671116a6557b9921`, tree `d83c801a6413910bc08b33734621e0fb4dcf3acc`, uses `io.CopyN` for exact 65,536/65,537 boundary writes; current race and oldstable each passed both cases 200 times, followed by complete exact dual-Go local gates. Push `29660519282` passed 22 jobs and failed only on external GitHub DNS during Homebrew update; PR `29660520860` passed the Helper boundary but exposed that `cache:lru | normal` also appears during loading. The pending correction makes copy/move/rename/delete all wait for `READ-ONLY | sort:name`; the complete flow passed current race 20 iterations and exact oldstable 10, followed by complete exact current `make ci` and exact oldstable `make check`. No timeout, diagnostic assertion, support scope, production Helper, or Level 2 policy changed; corrected exact-SHA Hosted evidence remains open.
