...
**Current checkpoint**: 唯一基线仍为 `312bcccbcbd54246bbe5ff9babf4f14560449176`、tree `e0316c286ce11512cb0b92c917fa29b80f9e3305`。M6.1 已完成，M6.2 historical/restore、Job-history retention、Helper state lifecycle、deterministic package、dependency closure、Homebrew renderer 与 public Helper management 前序门禁已绿；JOB-010 为 `Verified`。release-source commit `53fceb0bf672607ba52d47550a65b095f3e0c76e` 冻结 exact four archive/manifest/signature 名称与不可变 URL，只在 metadata current-policy、preliminary consent、fresh probe、high-water 后按 context 惰性流式读取 exact USTAR，compressed/expanded hard limits、立即 unlink 的 0600 file、每次 reopen size/SHA-256 与 bind/Installer policy 复核均通过。owner composition SHA `9ee5afaa3290ccd2d6c00b0aee58563caae73844` 已接入私有 Host alias→opaque EndpointID registry、restricted daemon RPC、LifecycleManager/JobStore removal recovery、fresh SFTP/OpenSSH owners 与 exact 0700 MKDIR；production verifier/policy 为空时仍在 mutation runtime 前关闭。native lifecycle SHA `99905179e1eac23819a8b7f25edffc07c29cf653` 进一步在 Ubuntu 22.04/24.04 与 macOS ARM/Intel 从 fresh owner-private HOME 安装 binary/man/bash-zsh-fish completion，验证 source↔installed version、daemon/Job start/status/stop，精确卸载 owned files 并保留 SQLite；PR 四平台全部执行成功，push Ubuntu 22.04/macOS ARM 成功，另三个失败均为 lifecycle 前既有 Helper process-reader/heartbeat 时序 fixture 且有同 SHA 对侧成功 companion。preview archive 的显式连接和无参数 host/workspace picker 都已有 fresh-user Hosted 证据。项目 LICENSE、完整 NOTICE、pinned-cache/public channel、final signed/unsigned archives、production signing/notary/Helper/Level 2 仍开放；12 个 Stage 6 退出条件仍全部开放，production Helper distribution 与 production Level 2 保持 **CLOSED**。

**Latest M6.2 increment**: no-argument picker SHA `fbddcaa0208d2fca5c285162c5c9ffbf7fa8d318`、tree `ad8df693973f93f5604cc56996c2f9786019eef2` 在 fresh owner-private HOME 中直接以空参数启动归档 binary，展示 SSH-config aliases，选择独立 chroot `recovery-picker` 并列出其远端 marker；随后保存双远端 workspace，再次空参数启动、筛选并重开该 workspace。两套 `quality`→`auth-integration` 都验证同运行 SHA-bound bundle 后完成 OpenSSH/TUI 与 Kerberos；PR [29638718437](https://github.com/TyrantLucifer/awsome-sftp-cli/actions/runs/29638718437) 24/24 全绿，push [29638717696](https://github.com/TyrantLucifer/awsome-sftp-cli/actions/runs/29638717696) 的关键 quality/auth 全绿，唯一 oldstable macOS Intel failure 是既有 Helper heartbeat visibility fixture 且同 SHA PR companion 成功，未 rerun。该切片关闭无配置 first-run host/workspace picker 的预览归档演练，不等于真实公开渠道；pinned-cache upgrade/failed-upgrade recovery、真实 LICENSE/NOTICE、final archives、签名/公证及 production Helper/Level 2 继续开放。
