...
**Current checkpoint**: 唯一基线仍为 `312bcccbcbd54246bbe5ff9babf4f14560449176`、tree `e0316c286ce11512cb0b92c917fa29b80f9e3305`。M6.1 已完成，M6.2 historical/restore、Job-history retention、Helper state lifecycle、deterministic package、dependency closure、Homebrew renderer 与 public Helper management 前序门禁已绿；JOB-010 为 `Verified`。release-source commit `53fceb0bf672607ba52d47550a65b095f3e0c76e` 冻结 exact four archive/manifest/signature 名称与不可变 URL；owner composition SHA `9ee5afaa3290ccd2d6c00b0aee58563caae73844` 接入私有 alias registry、restricted daemon RPC、durable removal recovery 与 fresh SFTP/OpenSSH owners，同时 production trust 为空时仍在 mutation runtime 前关闭。native lifecycle、preview archive picker 与 pinned-cache recovery 均有 exact-SHA 证据。第三方 NOTICE SHA `197ff6f12135be670df40bd5f0dfe7e8c72fe483` 现对 20 个 runtime modules 做 declared/resolved/replacement/SPDX 一一覆盖，并以 SHA-256 绑定每份 reviewed redistribution source；`modernc.org/libc`/`memory` 的额外文本完整内联，`make notice-check`、CI regeneration/byte compare 与 archive packaging 均 fail closed。exact current/oldstable 本地门禁和两套 Hosted quality 的 NOTICE/packaging 路径通过。该技术闭包不替项目选择许可证；项目 LICENSE、真实 public channel、final signed/unsigned archives、production signing/notary/Helper/Level 2 仍开放；12 个 Stage 6 退出条件仍全部开放，production Helper distribution 与 production Level 2 保持 **CLOSED**。

**Latest M6.2 increment**: reviewed third-party NOTICE implementation SHA `197ff6f12135be670df40bd5f0dfe7e8c72fe483`、tree `e1d203edcb820c332a98650cd6256bf6f5872a43` 通过 exact current Go 1.26.5 `make ci` 与 exact oldstable Go 1.25.12 `make check`。Hosted push [29641186074](https://github.com/TyrantLucifer/awsome-sftp-cli/actions/runs/29641186074) 为 22 success / 1 failure / 1 dependency-skipped，PR [29641186949](https://github.com/TyrantLucifer/awsome-sftp-cli/actions/runs/29641186949) 为 21 success / 1 failure / 2 dependency-skipped。push quality 完整生成、比较并打包真实 NOTICE；PR quality 也完成这些步骤后才在冻结 Stage 5 daemon 的既有 readiness 轮询失败。push native macOS Intel 唯一失败是已记录的 exact-stderr-cap reader race（0 vs 65,536）；同 SHA PR Intel companion 通过。未 rerun、未修改 timeout/assertion/workflow。完整 redistribution NOTICE 已技术闭合；项目 LICENSE 仍由 owner/legal 选择，下一独立切片继续 truthful pre-publication channel。
