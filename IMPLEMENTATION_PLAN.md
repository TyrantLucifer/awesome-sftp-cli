...
**Current checkpoint**: 唯一基线仍为 `312bcccbcbd54246bbe5ff9babf4f14560449176`、tree `e0316c286ce11512cb0b92c917fa29b80f9e3305`。M6.1 已完成，M6.2 historical/restore、Job-history retention、Helper state lifecycle、deterministic package、dependency closure、Homebrew renderer 与 public Helper management 前序门禁已绿；JOB-010 为 `Verified`。release-source commit `53fceb0bf672607ba52d47550a65b095f3e0c76e` 冻结 exact four archive/manifest/signature 名称与不可变 URL；owner composition SHA `9ee5afaa3290ccd2d6c00b0aee58563caae73844` 接入私有 alias registry、restricted daemon RPC、durable removal recovery 与 fresh SFTP/OpenSSH owners，同时 production trust 为空时仍在 mutation runtime 前关闭。native lifecycle、preview archive 显式连接和无参数 host/workspace picker 均有 fresh-user Hosted 证据。pinned-cache recovery SHA `925b22eb6a743816abcd11d1e552a7af1ce2992a` 又由冻结 Stage 5 production owners 创建 schema v3 并发布 exact pinned-offline cache entry，current owner 在升级前、受控持久化 failed-attempt 边界、degraded 默认拒绝、显式 `--resume-migration` 到 v4、冻结 Stage 5 read-only rollback 与 current restart 后逐次复验同一 entry/blob identity 和字节；两套 Hosted `quality` 均通过。该 failure helper 是对生产 attempt/verified-backup/resume 边界的受控持久化演练，不冒充自然失败的 v4 SQL statement；实际 statement rollback 仍由 migration runner tests 覆盖。项目 LICENSE、完整 NOTICE、真实 public channel、final signed/unsigned archives、production signing/notary/Helper/Level 2 仍开放；12 个 Stage 6 退出条件仍全部开放，production Helper distribution 与 production Level 2 保持 **CLOSED**。

**Latest M6.2 increment**: pinned-cache recovery implementation SHA `925b22eb6a743816abcd11d1e552a7af1ce2992a`、tree `540681e14deec18b6d86a3d74f738b129305db18` 通过 exact current Go 1.26.5 `make ci`、exact oldstable Go 1.25.12 `make check` 和受信根真实 lifecycle。Hosted push [29639963220](https://github.com/TyrantLucifer/awsome-sftp-cli/actions/runs/29639963220) 为 21 success / 2 failure / 1 dependency-skipped，PR [29639964534](https://github.com/TyrantLucifer/awsome-sftp-cli/actions/runs/29639964534) 为 22 success / 1 failure / 1 dependency-skipped；两边 `quality`、auth、oldstable、build 和 reproducibility 均成功。两条 ARM macOS failure 都是既有 Helper heartbeat visibility race；push Intel macOS 另在既有 remote-command cancellation deadline fixture 失败，而同 SHA PR Intel companion 成功。新增 integration packages 在这些 runner 上也通过；未 rerun、未放宽 timeout/assertion。pinned-cache upgrade/failed-upgrade recovery rehearsal 已关闭，下一独立切片转向真实项目 LICENSE/NOTICE 与 truthful pre-publication channel；production trust 仍不阻断这些工作。
