...
**Current checkpoint**: 唯一基线仍为 `312bcccbcbd54246bbe5ff9babf4f14560449176`、tree `e0316c286ce11512cb0b92c917fa29b80f9e3305`。M6.1 已完成；M6.2 的 migration/package/native/preview Homebrew 独立切片已有 exact-SHA 证据，真实 public channel、项目 LICENSE 与 final protected bytes 仍开放。M6.3 foundation SHA `ae172d9bb419bc6b0b05cf3ef3467258496d463f` 建立 shared export redaction 与 deterministic bounded support-bundle core；doctor SHA `7aa6f20d1cada28064cfd4f169442ef219a9b1a8` 及 docs SHA `d345c362ad8b6ea8c67b09a4e351ad27e929965a` 已完成本地双 Go 和 same-SHA Hosted 分类。support-bundle SHA `bfc59c8c0478a7043b8287f52d31dd3477b30554` 又交付 public preview/create、真实八源 composition、重新组合 consent、private 0600 no-replace/no-follow publication、fsync 与失败/取消 cleanup；raw messages/IDs/paths/argv/locations/errors/content 不进入 bundle。focused normal/race/vet/lint、isolated-HOME no-creation smoke、exact current `make ci` 与 exact oldstable `make check` 已通过，本批 Hosted 尚待。完整跨日志/状态/认证 secret matrix、corrupt-state matrix、compatibility 与 finding ledger 仍开放，因此 SEC-012/OBS-009/OBS-010 继续 `In Progress`。12 个 Stage 6 退出条件仍全部开放，production Helper distribution 与 production Level 2 保持 **CLOSED**。

**Latest M6.3 increment**: doctor docs SHA `d345c362ad8b6ea8c67b09a4e351ad27e929965a` 的 push/PR 已完整观察；仅出现既有 Helper heartbeat/stderr reader visibility timing，same-SHA companion/step 证明新 doctor/statefs/OpenSSH inspection、CLI、文档、packaging 与 auth 无回归，未 rerun 或弱化门禁。随后 support-bundle RED 仅因 `Publish`、runtime/snapshot/composer 缺失而编译失败。GREEN SHA `bfc59c8c0478a7043b8287f52d31dd3477b30554` 固定 manifest+八源 preview、exact digest consent、create 时重新组合、4 MiB deterministic archive，以及绝对 canonical output、private parent、`O_EXCL|O_NOFOLLOW`、0600、chunked cancellation、file/dir sync 和 partial cleanup。配置仅导出 shape；diagnostic/Job 不导出 message、IDs、paths、argv、location/error/content；daemon 缺失/失败为安全空摘要且不自动启动。focused race/vet/lint、isolated HOME preview no-creation smoke、exact current Go 1.26.5 full `make ci` 与 exact oldstable Go 1.25.12 full `make check` 均首轮通过；本批 exact-SHA Hosted 是下一门禁。
