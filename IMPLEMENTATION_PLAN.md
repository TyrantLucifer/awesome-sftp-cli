...
**Current checkpoint**: 唯一基线仍为 `312bcccbcbd54246bbe5ff9babf4f14560449176`、tree `e0316c286ce11512cb0b92c917fa29b80f9e3305`。M6.1 已完成，M6.2 historical/restore、Job-history retention、Helper state lifecycle、deterministic package 与 dependency closure 前序门禁已绿；JOB-010 为 `Verified`。installed lifecycle evidence SHA `d625a677f59e6e21d55516d2533db34c43d516fa` push 24/24 并完整通过受信根 lifecycle；PR 仅初始 Stage 5 readiness 超过 10 秒界限，同 SHA 伴随证据将其分类为 runner timing instability，未 rerun。公共 Helper implementation `49f39401c1693a92917a77efae1ab0a41175237b` 增加 install/upgrade、literal stable-home consent、任意 manifest/artifact/path pre-runtime 拒绝、human/JSON stable configuration exit 3 与 help/man/bash/zsh/fish parity；protected production trust closed 时 connector/RPC/SSH/probe/write 均为 0。focused GREEN、current full `make ci`、exact oldstable full `make check` 与 exact clean real-binary/fresh-HOME smoke 已绿，Helper batch Hosted 待执行。项目 LICENSE、完整 NOTICE、native macOS/channel install、production signing/notary/Helper/Level 2 仍开放；12 个 Stage 6 退出条件仍全部开放，production Helper distribution 与 production Level 2 保持 **CLOSED**。
