...
**Current checkpoint**: 唯一基线仍为 `312bcccbcbd54246bbe5ff9babf4f14560449176`、tree `e0316c286ce11512cb0b92c917fa29b80f9e3305`。M6.1 已完成；M6.2 的 migration/package/native/preview Homebrew 独立切片已有 exact-SHA 证据，真实 public channel、项目 LICENSE 与 final protected bytes 仍开放。M6.3 doctor 与 support-bundle public preview/create/private publication 已交付。support-bundle evidence SHA `496356908990d4bb5d9473acad754fd8ae7217ca` 的 push/PR 均完成 22 success / 1 known Helper visibility failure / 1 dependency-skipped，失败各有 same-SHA 对侧平台 companion 且新包在失败 job 内通过，未 rerun。hardening SHA `ba334b8d8968f5b09c91c0185996994b9a307ff4` 再将持久日志值与导出 build/platform/doctor/diagnostic/Job/database/daemon 字段收紧到 semantic registries/allowlists；safe-shaped 八类 secret corpus 在 persistent JSON 与 expanded archive 中均为 0-hit，corrupt config/database/cache/daemon 只输出稳定码且 bytes 不变。focused normal/race/vet、exact current `make ci` 与修正本地 tmp 环境后的 exact oldstable `make check` 通过。实际 auth-artifact/Helper secret 扫描、compatibility、troubleshooting 与 finding ledger 仍开放，因此 SEC-012/OBS-009/OBS-010 继续 `In Progress`。12 个 Stage 6 退出条件仍全部开放，production Helper distribution 与 production Level 2 保持 **CLOSED**。
