...
Stage 0–5 已完成；各阶段均由完整本地门禁、exact-SHA Hosted evidence、文档真相链和独立冷启动审计闭环。Stage 5 已合并，Stage 6 在固定分支 `codex/stage6-hardening-release` 从 verified exact-main `312bcccbcbd54246bbe5ff9babf4f14560449176` 开始。M6.1 已完成；M6.2/M6.3/M6.4 的本地、Hosted、物理 100 GiB、内部预览与最终发行证据按 Stage 6 execution plan 继续累积。当前 M6.3 transition-hardening 的双 Go 本地门禁通过，PR `29683309618` 首轮 25/25，push `29683308347` 对两条已分类既有时序失败各最小重跑一次后 25/25；同一矩阵也让要求的 Ubuntu 24.04 ARM64 原生 lifecycle 获得双工作流证据。下一兼容批次把真实 Ubuntu 24.04 ProFTPD `mod_sftp` 接入 auth job：精确捕获并在任何 fixture mutation 前重检 server/package 版本，随后以生产 Provider/Planner/Worker 和 exact internal-preview TUI 验证 Level 0 与 extension-gated durable transfer。当前聚焦合同、docscheck、actionlint、shell syntax、exact current Go 1.26.5 `make ci` 与 exact oldstable Go 1.25.12 `make check` 已通过；Hosted exact-head 仍是接受门禁。PLAT-005/REL-006 保持 `In Progress`，macOS Kerberos、broader vendor coverage、final protected bytes、public channel、Production Helper 和 Level 2 继续开放。
...
**Current checkpoint**: 唯一基线仍为 `312bcccbcbd54246bbe5ff9babf4f14560449176`、tree `e0316c286ce11512cb0b92c917fa29b80f9e3305`。M6.1 已完成；M6.2 的 migration/package/native/preview Homebrew 独立切片已有 exact-SHA 证据，真实 public channel、项目 LICENSE 与 final protected bytes 仍开放。M6.3 real-auth scan SHA `fd5ebe499331f5697cdc5eb1e238d12e939ee53a` 已有 dual-workflow 证据。OpenSSH floor/current SHA `e7733ec23ec1903dbdc41a926db20fe6defcccc7`/`a7f3ddd8a99df973518140de7eae45e295ae6ea2` 与 Kerberos binding SHA `14e5ec4e63c70fb93ed93d15adbe7fdf7cce5d5e` 均通过 exact current Go 1.26.5 `make ci`、exact oldstable Go 1.25.12 `make check` 和双工作流 auth gates；版本化矩阵精确记录 `OpenSSH_8.9p1 Ubuntu-3ubuntu0.16`、`OpenSSH_9.6p1 Ubuntu-3ubuntu13.18` 与 `Kerberos 5 version 1.20.1`，且 retained version 均在任何测试账户/服务变更前重检。Transition-hardening SHA `3bfbd174a7cf264847d8d3bf0ff6e0e5ad62aff0` 让 `daemon stop` 只在 Socket 消失且实例锁已释放后报告成功，并以真实独立 Helper 子进程覆盖 current、future minimum client、future protocol、future exact-artifact mismatch 及每次失败后的 current 恢复；PR `29683309618` 首轮 25/25，push `29683308347` unchanged failed-only rerun后 25/25。下一批以真实 ProFTPD `mod_sftp`、精确 pre-mutation 版本绑定、生产 Provider/Planner/Worker durable transfer 与 exact preview TUI 补第三方 server 证据；exact current `make ci` 与 oldstable `make check` 已通过，Hosted exact-head 尚待批次末门禁。PLAT-009 已 `Verified`，REL-006/011/012 为 `In Progress`；macOS Kerberos、broader vendor SFTP、public-channel rollback、production-Helper artifact 扫描与 final review 仍开放。12 个 Stage 6 退出条件仍全部开放，production Helper distribution 与 production Level 2 保持 **CLOSED**。

**Latest M6.3 increment**: Three canonical Kerberos RED slices failed only on the absent exact `/usr/bin/klist -V` capture, retained-version dataflow into the real harness, and pre-mutation drift rejection. GREEN SHA `14e5ec4e63c70fb93ed93d15adbe7fdf7cce5d5e` accepts only `Kerberos 5 version *`, retains the exact value, passes it through `sudo env`, and rejects byte drift before user/KDC mutation; provenance policy freezes the new step and dataflow. Exact current/oldstable full local gates pass. Push/PR auth jobs `88105308661`/`88105369623` both record `Kerberos 5 version 1.20.1`, pass binding, and complete GSSAPI expiry/recovery. Residual Hosted failures are known macOS PTY/Helper timing and external Homebrew API availability, with no rerun or weakened gate. REL-006 moves to `In Progress`, not `Verified`, because the documented untested combinations remain open.

**REL-007 increment**: Three canonical review-contract RED slices required all eight review domains, executable negative-test anchors, and explicit open production/final-review boundaries. GREEN SHA `cc6559e41572aa21a7fc249fe0086fe4df50fe27`, tree `6d1ead4f23de7a1ac344d959f0b8cb75c004ab7f`, records stable coverage IDs and scoped `reviewed`/`partial` dispositions; exact current/oldstable full local gates pass. Push/PR `29655143700`/`29655145234` each finished 22 success / 1 failure / 1 dependency-skipped. The residual failures are the existing Helper stderr-overflow reader race and external Homebrew API availability, with exact same-SHA companion success and no rerun or weakened gate. REL-007 moves to `In Progress`, not `Verified`; production Helper/Level 2, protected final artifacts, hardware/platform gaps, and final independent security review remain open.

**REL-009 increment**: Four canonical documentation RED slices required the user/operations index, exact first-run SSH/Kerberos and CLI path, bounded diagnostic/Job/Helper/direct fallback runbook, and upgrade/rollback/withdrawal lifecycle. GREEN SHA `19e8ab73d5f90a51815634b9b36112f86711f372`, tree `ceb8e749ac11c873b703d3bee3f3168cb9f5fce2`, adds the missing first-run, operations, and upgrade documents, corrects stale troubleshooting command spellings at the generated registry source, and binds every required domain while keeping protected production boundaries closed. Exact current Go 1.26.5 `make ci`, exact oldstable Go 1.25.12 `make check`, focused docscheck, and public help/man/completion parity pass. Hosted push/PR `29656202301`/`29656203801` passed both quality/auth jobs; their residual existing OpenSSH-expansion and Helper-process visibility timing failures all have same-SHA companion coverage, with no rerun or weakened gate. REL-009 remains `In Progress`, not `Verified`; the independent exact-RC new-user path remains open.

**REL-008 increment**: Five canonical RED slices required an exact ordered gate catalog, complete successful candidate identity binding, physical/isolated/soak anti-substitution metrics, exact candidate push/PR plus post-merge main semantics, and a strict bounded CLI record reader. GREEN SHA `bad687f67789960bac426bd9414cfbcc5b859b49`, tree `bfa026b52e2a55738f023623093347c5ee78d21e`, adds `amsftp-release-gate-record-v1` and the operator validator, rejecting mixed SHA/tree, missing/duplicate/wrong-kind gates, failed/skipped Hosted jobs, synthetic physical 100 GiB, same-process or non-network Level 2, incomplete soak acceptance, and fewer than two exact-candidate reviews. Exact current/oldstable local gates pass. PR `29657551397` passed 24/24; push `29657550405` passed both quality/auth paths and its two existing Helper process-reader timing failures have same-SHA PR Intel companion success, with no rerun or weakened gate. REL-008 remains `In Progress`, not `Verified`; no RC record exists and real exact-RC/protected evidence remains open.

**REL-010 increment**: Four canonical RED slices require a frozen 23-row Stage 6 feature set, terminal final statuses, ADR-backed `Deferred`/`Removed` decisions, the exact ordered 12 exit criteria, and an explicit final-only CLI mode. GREEN SHA `181f8a8fbf083c5dadf450373981693e82ecdf74` adds `docscheck --release` without changing ordinary docscheck. It rejects missing/unexpected/stage-drifted features, `Planned`/`In Progress`/`Implemented` final rows, unsupported decisions, changed/reordered/extra/missing criteria, and unchecked exits. Exact current/oldstable local gates and both 24/24 Hosted workflows pass. The current tree reports exactly 21 nonterminal rows plus 12 unchecked exits, so REL-010 is `In Progress`, not release-passing.

**Hosted timing-stability increment**: SHA `60255dc8dc5e4732ae0fe55ac11ca860390bc81c` established Helper terminal joining, deterministic tail polling, and an initial Stage 2 barrier. SHA `573aeefcaa488d73a7b5186a46ef03f87c1d8416` restored the resident exact-stderr child; SHA `60ff9080812e0e47f720d102671116a6557b9921` uses `io.CopyN` for exact 65,536/65,537 writes and its push/PR each finished 22 success / 1 failure / 1 skip while proving the writer fix. Loaded-selection SHA `2f4ed536ca2553e1bd266ed66fd275b664a00690`, tree `7961527cf2273dc55a88f8409b6af989c80c97b8`, routed copy/move/rename/delete through one barrier and passed exact dual-Go local gates, but push `29661476223` and PR `29661477838` exposed that fixed `READ-ONLY | sort:name` excludes loaded remote capability rows. Final-screen correction SHA `98a3315070f12b97fe43fa1a79dcdfc4e7428701`, tree `6c2e1dda9cb1768d98498458dcbe25adfe684aea`, requires the cursor-selected filename and absence of `READ-ONLY | loading`; deterministic local/remote capability screens, the complete local PTY, exact dual-Go local gates, push `29662965862`, and PR `29662967034` all pass, with each Hosted run at 24/24. A retained temporary-sshd stress attempt failed only after the isolated transport became unavailable and was not rerun. No timeout, diagnostic assertion, support scope, production Helper, or Level 2 policy changed; this remains pre-RC hardening.
