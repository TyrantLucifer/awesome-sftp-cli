...
**Current checkpoint**: 唯一基线仍为 `312bcccbcbd54246bbe5ff9babf4f14560449176`、tree `e0316c286ce11512cb0b92c917fa29b80f9e3305`。M6.1 已完成；M6.2 的 13-source historical closure 与 verified-restore commit `16cfd914b17239203259283e6324b3e7a50ba422` 已通过本地双 Go 和 exact-SHA push/PR Hosted 门禁，后者的 PR 首轮已知 Helper reader/PTY timing failures 在同 SHA 无代码重跑后通过。当前批次冻结 SQLite head 4，并完成固定 1.0 Job-history policy、单例持久 claim、有 WAL/事务上限的分块清理、进程死亡续接、edit/cache recovery-reference 保护和 daemon 启动 owner；focused current、完整 current Go 1.26.5 `make ci` 与 exact Go 1.25.12 `make check` 全绿，exact-SHA Hosted 尚待执行。JOB-010/REL-003 为 `In Progress`；12 个 Stage 6 退出条件仍全部开放，production Helper distribution 与 production Level 2 保持 **CLOSED**。
