...
**Current checkpoint**: 唯一基线仍为 `312bcccbcbd54246bbe5ff9babf4f14560449176`、tree `e0316c286ce11512cb0b92c917fa29b80f9e3305`。M6.1 已完成；M6.2 的 migration/package/native/preview Homebrew 独立切片已有 exact-SHA 证据，真实 public channel、项目 LICENSE 与 final protected bytes 仍开放。M6.3 foundation SHA `ae172d9bb419bc6b0b05cf3ef3467258496d463f` 建立 shared export redaction policy、固定顺序/稳定 code/severity/remediation 且丢弃 raw cause 的 doctor report，以及只接受 reviewed JSON、固定白名单与 sensitivity、preview consent digest、单次 byte snapshot、512 KiB/file、3 MiB expanded、4 MiB compressed 和 deterministic USTAR/gzip 的 support-bundle core。config effective export 与 persistent diagnostics 已复用同一 policy；seeded host/path/token 即使伪标为 reviewed 也 fail closed。exact current Go 1.26.5 `make ci` 与 oldstable Go 1.25.12 `make check` 通过。CLI、真实 system probes、私有写盘/cleanup 与完整 secret matrix 尚未完成，因此 SEC-012/OBS-009/OBS-010 仅进入 `In Progress`。12 个 Stage 6 退出条件仍全部开放，production Helper distribution 与 production Level 2 保持 **CLOSED**。

**Latest M6.3 increment**: final foundation SHA `ae172d9bb419bc6b0b05cf3ef3467258496d463f`、tree `fc865f75271cbc50162d867c43f15f9d270a70f9` 通过两套本地完整门禁。初始 full current gate 在所有测试/docs/tidy/verify 通过后由 lint 发现两个本批 staticcheck selector 问题，修正后从头通过；后续安全复核以 seeded-secret RED 发现 claimed-reviewed JSON 仍可携带明显 secret，最终实现加入有界 iterative JSON review 并再次从头通过 current/oldstable。Hosted push [29645261188](https://github.com/TyrantLucifer/awsome-sftp-cli/actions/runs/29645261188) 24/24；PR [29645262834](https://github.com/TyrantLucifer/awsome-sftp-cli/actions/runs/29645262834) 22 success / 1 failure / 1 dependency-skipped，唯一失败是冻结 Stage 2 PTY delete 在 listing selection 刷新前触发，同 SHA push Intel 完整通过。新增包在失败 job 均通过；未 rerun、未弱化门禁。中间 SHA `b501f58` 的 workflows 被最终安全修正提交按 concurrency 正常取消，不计证据。CLI 和真实 probe composition 留给下一批。
