...
**Current checkpoint**: 唯一基线仍为 `312bcccbcbd54246bbe5ff9babf4f14560449176`、tree `e0316c286ce11512cb0b92c917fa29b80f9e3305`。M6.1 已完成；M6.2 的 migration/package/native/preview Homebrew 独立切片已有 exact-SHA 证据，真实 public channel、项目 LICENSE 与 final protected bytes 仍开放。M6.3 doctor、support-bundle 与 semantic-secret hardening 已交付；hardening 文档 SHA `b5293d7dd12587c1e6c326a31a60ec55b21051ad` 的 push/PR 均 24/24。compatibility/security contract SHA `6476bfc8b80e1aeefb10bc02cbc30eb6740e35a6` 冻结 26 行 environment evidence status、全部 11 个 doctor/15 个 domain code 的 troubleshooting map、threat model 与首条 fixed finding ledger。focused normal/race/vet/docscheck 与 exact oldstable Go 1.25.12 `make check` 通过；current Go 1.26.5 ordinary/contract/docs/lint/fuzz/supply-chain/four-build 均通过，race 仅命中既有 Helper `stderr_overflow` reader-visibility timing，未 rerun。首轮 inherited `umask 077` 把 deliberate unsafe fixture 收紧成安全 mode，显式恢复文档化 `022` 后 focused/gate 通过，未修改代码、断言或 timeout。实际 auth-artifact/production-Helper secret 扫描、exact-SHA Hosted 与 final review 仍开放，因此 SEC-012/OBS-009/OBS-010 继续 `In Progress`。12 个 Stage 6 退出条件仍全部开放，production Helper distribution 与 production Level 2 保持 **CLOSED**。

**Latest M6.3 increment**: canonical RED first failed on absent `supportBundleGatherRuntime`/`gatherSupportBundleSourcesWithRuntime` and proved the existing logger persisted the safe-shaped `askpass_answer_canary_7f3a` as component/event. GREEN SHA `ba334b8d8968f5b09c91c0185996994b9a307ff4` adds a read-only injectable gather boundary, reviewed diagnostic registries, field-specific export validators, corrupt-state stable summaries, byte-preservation tests, and source plus expanded-archive corpus scans. A preliminary shell without Go was environment discovery, not RED evidence. Current full `make ci` passed first attempt. Oldstable first reached vet with `/tmp` full; a second attempt intentionally moved TMPDIR to `/data00` and correctly triggered owner-ancestor plus filesystem-order fixtures. After deleting only this batch's exact disposable `/tmp` root, keeping Go cache external, and restoring test TMPDIR to `/tmp`, unchanged exact Go 1.25.12 `make check` passed. No code, assertion, timeout, permission check, or traversal-order contract changed.
