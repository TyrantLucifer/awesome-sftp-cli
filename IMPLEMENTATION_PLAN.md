...
**REL-010 increment**: Four canonical RED slices require a frozen 23-row Stage 6 feature set, terminal final statuses, ADR-backed `Deferred`/`Removed` decisions, the exact ordered 12 exit criteria, and an explicit final-only CLI mode. GREEN adds `docscheck --release` without changing ordinary docscheck. It rejects missing/unexpected/stage-drifted features, `Planned`/`In Progress`/`Implemented` final rows, unsupported decisions, changed/reordered/extra/missing criteria, and unchecked exits. The current tree reports exactly 21 nonterminal rows plus 12 unchecked exits, so REL-010 is `In Progress`, not release-passing.
