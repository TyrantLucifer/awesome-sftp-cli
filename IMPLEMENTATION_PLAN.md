...
**Current checkpoint**: 唯一基线仍为 `312bcccbcbd54246bbe5ff9babf4f14560449176`、tree `e0316c286ce11512cb0b92c917fa29b80f9e3305`。M6.1 已完成，M6.2 historical/restore、Job-history retention 与 Helper lifecycle 前序门禁已绿；JOB-010 为 `Verified`，REL-003 仍为 `In Progress`。确定性公共打包与 clean-home 生命周期 commit `958dec07addbe12c0c02404af4baf90558286909`、tree `d567739f32e89ca17ca4d2c3c6502e0579dc4c0d` 的 push `29629620491`/PR `29629622018` 均首轮 24/24 全绿。依赖闭集 commit `99db0e822b80d01519191e9bd13a894794c018c2`、tree `05beb8459b9f2c158c090f91747f833a21d06f17` 也在 push `29630238252`/PR `29630239460` 首轮各 24/24 全绿，把四目标实际 build info 固定为 20-module union（Darwin 20、Linux 18），逐 target 精确核验 path/version/h1/replacement，并严格解析 SPDX。当前五切片批次把 canonical man page 纳入 archive、从安装二进制生成三种 shell 补全、用冻结 Stage 5 二进制创建 v3 后由 current 升级至 v4、验证旧 daemon 对 newer DB 只降级且不改字节，并执行 exact-owned-file uninstall 保留数据库；focused GREEN、current Go 1.26.5 `make ci` 与 exact oldstable Go 1.25.12 `make check` 首轮通过，implementation commit `0fdcb9cbcc734e9674d2455ec8123eacca722498`/tree `5f7bf0b208f6fce1e01252dc730d5c5014884f68` 的 exact clean 双打包与完整 lifecycle smoke 也已通过，Hosted 尚待执行。项目 LICENSE、完整 NOTICE、native macOS/channel install、production signing/notary/Helper/Level 2 仍开放；12 个 Stage 6 退出条件仍全部开放，production Helper distribution 与 production Level 2 保持 **CLOSED**。
