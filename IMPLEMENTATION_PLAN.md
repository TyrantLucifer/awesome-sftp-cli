...
**Current checkpoint**: 唯一基线仍为 `312bcccbcbd54246bbe5ff9babf4f14560449176`、tree `e0316c286ce11512cb0b92c917fa29b80f9e3305`。M6.1 已完成，M6.2 historical/restore、Job-history retention、Helper state lifecycle、deterministic package 与 dependency closure 前序门禁已绿；JOB-010 为 `Verified`。installed lifecycle evidence SHA `d625a677f59e6e21d55516d2533db34c43d516fa` push 24/24 并完整通过受信根 lifecycle；PR 仅初始 Stage 5 readiness 超过 10 秒界限，同 SHA 伴随证据将其分类为 runner timing instability，未 rerun。公共 Helper evidence SHA `2f39983ebb693b96fb9c97f81580e1d0b6521bb0` 的 push quality/full lifecycle 以及双 workflow app/docs/help/man/completion contract 均通过；四个失败各有同 SHA 成功 companion，分类为既知 readiness/PTY/heartbeat 时序不稳定，未 rerun、未改断言。Homebrew evidence SHA `4830bbecea709a356aa66044e3f77e12dc215395` 的 PR 24/24 全绿；push 通过 Homebrew/releasepack、lint/fuzz/supply-chain 和其余 22 jobs 后，仅 Stage 5 首次 daemon readiness 超界，同 SHA PR lifecycle 完整通过，分类为 runner/process timing instability，未 rerun。Helper public-surface evidence SHA `9c2c19185ede624d5d7905614bb0bc6112b90a19` 的双 quality 和完整生命周期通过；push ARM 与 PR Intel 各自的 heartbeat/stderr/PTY 时序失败均有同 SHA 对侧平台成功 companion，compare 仅依赖跳过，分类为既有不稳定，未 rerun。release-source commit `53fceb0bf672607ba52d47550a65b095f3e0c76e` 冻结 exact four archive/manifest/signature 名称与不可变 URL，metadata current-policy 通过前 archive 读取为 0，且 preliminary consent 拒绝同样保持 0；通过 fresh probe/high-water 后才按 context 惰性下载，硬限制 compressed/expanded bytes，只接受 exact USTAR entry set，并把 binary 流式写入立即 unlink 的 0600 file。两次 drift-bound validation 与上传流分别重开并校验 size/SHA-256；bind 和 Installer 再验 current policy。初始只读审查对提前下载、signed-size 整块分配和 post-hoc expanded limit 给出 BLOCK，三项修复及 GNU/USTAR 负例后复审 APPROVE；修正与证据归档为 commit `d64ee1a5477c42ee4d9a8112e0172fa846a0d4ef`、tree `82ccbf9993a0a98329e06a7cebb182320fc49e46`。evidence SHA `6f6a1a5ca869bf59fa513965d4b19ea8ab099e85` push 24/24 全绿；PR 的 quality/Intel/Linux/build/reproducibility 均通过，current ARM 仅错过 Stage 2 异步 PTY 选择刷新，oldstable ARM 在 Stage 3 proof 已通过后清理 `killpg` 返回 `EPERM`，两者均有同 SHA push ARM 成功 companion，因此分类为既有 runner/PTY/process cleanup 时序不稳定，未 rerun、未弱化门禁。daemon lifecycle composition、state mutation 与 remote delete 仍未接入。项目 LICENSE、完整 NOTICE、native macOS/channel execution、production signing/notary/Helper/Level 2 仍开放；12 个 Stage 6 退出条件仍全部开放，production Helper distribution 与 production Level 2 保持 **CLOSED**。
