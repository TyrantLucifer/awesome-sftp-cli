...
**Current checkpoint**: 唯一基线仍为 `312bcccbcbd54246bbe5ff9babf4f14560449176`、tree `e0316c286ce11512cb0b92c917fa29b80f9e3305`。M6.1 已完成，M6.2 historical/restore 前序门禁已绿。当前批次冻结 SQLite head 4，并完成固定 1.0 Job-history policy、单例持久 claim、有 WAL/事务上限的分块清理、进程死亡续接、edit/cache recovery-reference 保护和 daemon 启动 owner。首个 candidate `2ec8a7d895f1303d8d0c5df821b74d7edd6d90b3` 因 Hosted 命中本批 daemon socket timing test-design defect 而拒绝，既有 Helper/fuzz/PTY 时序另行保留。修正把生产启动序列提为可直接验证的函数，恢复原 daemon fixture，并以 ordinary + race `-count=5` 证明 retention claim 在返回前完成；修正后的完整 current `make ci` 与 exact oldstable `make check` 已绿，Hosted 待跑。JOB-010/REL-003 为 `In Progress`；12 个 Stage 6 退出条件仍全部开放，production Helper distribution 与 production Level 2 保持 **CLOSED**。
