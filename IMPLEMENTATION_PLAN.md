...
**Current checkpoint**: 唯一基线仍为 `312bcccbcbd54246bbe5ff9babf4f14560449176`、tree `e0316c286ce11512cb0b92c917fa29b80f9e3305`。M6.1 已完成，M6.2 historical/restore 与 Job-history retention 前序门禁已绿；JOB-010 为 `Verified`，REL-003 仍为 `In Progress`。当前 Helper lifecycle 批次的 RED 在 state v2、exact resolver、durable removal API 缺失处编译失败；GREEN 已实现 v1→v2 mutation-before migration、parallel exact installed records、one-active persistent high-water、candidate handshake 后原子切换、旧 frozen Job exact backend resolution、reference-leased durable removal claim、response-loss restart continuation 与只清理 completed-uninstall known metadata 的 1..64 bounded reconciliation。focused ordinary/race `-count=3`、完整 current Go 1.26.5 `make ci` 与 exact oldstable Go 1.25.12 `make check` 均首轮通过；exact-SHA Hosted 尚待本批 commit/push，因此 HELP-013 仅为 `In Progress`。12 个 Stage 6 退出条件仍全部开放，production Helper distribution 与 production Level 2 保持 **CLOSED**。
