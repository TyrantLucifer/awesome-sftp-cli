...
**REL-008 increment**: Five canonical RED slices required an exact ordered gate catalog, complete successful candidate identity binding, physical/isolated/soak anti-substitution metrics, exact candidate push/PR plus post-merge main semantics, and a strict bounded CLI record reader. GREEN adds `amsftp-release-gate-record-v1` and the operator validator, rejecting mixed SHA/tree, missing/duplicate/wrong-kind gates, failed/skipped Hosted jobs, synthetic physical 100 GiB, same-process or non-network Level 2, incomplete soak acceptance, and fewer than two exact-candidate reviews. REL-008 moves to `In Progress`, not `Verified`; no RC record exists and real exact-RC/protected evidence remains open.
