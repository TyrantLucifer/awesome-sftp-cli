...
**Current checkpoint**: 唯一基线仍为 `312bcccbcbd54246bbe5ff9babf4f14560449176`、tree `e0316c286ce11512cb0b92c917fa29b80f9e3305`。M6.1 已完成；M6.2 的 migration/package/native/preview Homebrew 独立切片已有 exact-SHA 证据，真实 public channel、项目 LICENSE 与 final protected bytes 仍开放。M6.3 compatibility/security evidence SHA `6653a617045b1ed0c03c5b297dbcbd38db27116d` 的 PR 24/24。auth-artifact 初始 Hosted SHA `00ca5d8ecf5f4ab47d7150f3dbaf17163678fb80` 暴露 scan output 放在 login home 时被 public publication 私有目录门禁拒绝；修正 SHA `fd5ebe499331f5697cdc5eb1e238d12e939ee53a` 仅将工件移到既有 0700 `XDG_STATE_HOME`。focused 与 dual-Go full local gates 全绿；PR `29651141681` 24/24，push `29651140328` 的两个既有 macOS Helper timing failure 均有同平台 PR companion。push/PR auth job `88098085447`/`88098124518` 均通过，且每份日志恰有两条 scan pass marker，分别位于完整 OpenSSH/Askpass 与 externally-renewed Kerberos/GSSAPI matrix 之后。production-Helper artifact 扫描与 final review 仍开放，因此 SEC-012/OBS-009/OBS-010 继续 `In Progress`。12 个 Stage 6 退出条件仍全部开放，production Helper distribution 与 production Level 2 保持 **CLOSED**。
