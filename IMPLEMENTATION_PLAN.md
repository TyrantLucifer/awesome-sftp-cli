...
**Current checkpoint**: 唯一基线仍为 `312bcccbcbd54246bbe5ff9babf4f14560449176`、tree `e0316c286ce11512cb0b92c917fa29b80f9e3305`。M6.1 已完成；M6.2 的 migration/package/native/preview Homebrew 独立切片已有 exact-SHA 证据，真实 public channel、项目 LICENSE 与 final protected bytes 仍开放。M6.3 compatibility/security evidence SHA `6653a617045b1ed0c03c5b297dbcbd38db27116d` 的 PR 24/24；push 21 success / 2 known scheduler/Helper timing failure / 1 dependency-skipped，两个同平台 PR companion 均通过且新包在失败 job 内通过，未 rerun。auth-artifact harness SHA `403a8fb123942225fa2e0e7f29a8a74b68a0d7bd` 让真实 OpenSSH/Askpass/Kerberos matrix 创建、展开并扫描 private support bundle，覆盖 literal/path、credential SHA-256/base64 和 exact key/ccache/keytab copies；focused normal/race/vet/docscheck、exact current `make ci` 与 oldstable `make check` 全绿。首轮 current lint 报告新增测试的两处 gosec variable-path finding；固定 repo-owned 路径的 line-scoped 说明修复后从头全绿。真实 auth Hosted 执行、production-Helper artifact 扫描与 final review 仍开放，因此 SEC-012/OBS-009/OBS-010 继续 `In Progress`。12 个 Stage 6 退出条件仍全部开放，production Helper distribution 与 production Level 2 保持 **CLOSED**。
