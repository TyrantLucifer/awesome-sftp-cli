...
**Current checkpoint**: 唯一基线仍为 `312bcccbcbd54246bbe5ff9babf4f14560449176`、tree `e0316c286ce11512cb0b92c917fa29b80f9e3305`。M6.1 已完成，M6.2 historical/restore、Job-history retention、Helper state lifecycle、deterministic package、dependency/NOTICE closure、native lifecycle、preview archive picker 与 pinned-cache recovery 均有 exact-SHA 证据；JOB-010 为 `Verified`。最新 Homebrew preview SHA `ed7dcd46dab669b929a483767a97890776caf4b9` 在 Hosted macOS ARM/Intel 从本地 loopback assets 和本地 tap 完成 clean install 0.9.0、version、upgrade 1.0.0、formula test、forced all-version uninstall、Cellar removal、untap 与受检 server shutdown；exact current Go 1.26.5 `make ci` 和 oldstable Go 1.25.12 `make check` 也通过。push/PR 的其余失败均落在已记录的 Stage 5 readiness 或 Helper reader/heartbeat 时序夹具，并有同 SHA companion，不属于 Homebrew 回归，未 rerun、未弱化门禁。这只是 CI 内 truthful pre-publication equivalence：BSD-3-Clause placeholder 仅用于 harness，绝不是项目 LICENSE；没有公开 formula、GitHub Release、最终 protected bytes、签名/公证或 tag。项目 LICENSE 仍由 owner/legal 选择；12 个 Stage 6 退出条件仍全部开放，production Helper distribution 与 production Level 2 保持 **CLOSED**。

**Latest M6.2 increment**: Homebrew preview implementation SHA `ed7dcd46dab669b929a483767a97890776caf4b9` 通过 exact current Go 1.26.5 `make ci` 与 exact oldstable Go 1.25.12 `make check`。Hosted push [29643786050](https://github.com/TyrantLucifer/awsome-sftp-cli/actions/runs/29643786050) 的 macOS ARM/Intel 均完成同 SHA 本地 tap 全生命周期；PR [29643787252](https://github.com/TyrantLucifer/awsome-sftp-cli/actions/runs/29643787252) 的 ARM 也完整通过，Intel 在进入 Homebrew 前命中既有 heartbeat fixture，而同 SHA push Intel 已通过。push oldstable Intel 的 exact-stderr-cap 与 PR quality/Intel 的 readiness/heartbeat 失败均有同 SHA 成功 companion，分类为既有时序不稳定。未 rerun、未修改 timeout/assertion/workflow。该 preview harness 技术闭合但不构成公开渠道或 LICENSE 选择；下一独立工作进入 M6.3 doctor/support-bundle，真实 public channel 留待最终受保护 bytes。
