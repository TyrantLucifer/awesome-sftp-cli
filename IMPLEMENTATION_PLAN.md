...
Stage 0–5 已完成；各阶段均由完整本地门禁、exact-SHA Hosted evidence、文档真相链和独立冷启动审计闭环。Stage 5 已合并，Stage 6 在固定分支 `codex/stage6-hardening-release` 从 verified exact-main `312bcccbcbd54246bbe5ff9babf4f14560449176` 开始。M6.1 已完成；M6.2/M6.3/M6.4 的本地、Hosted、物理 100 GiB、内部预览与最终发行证据按 Stage 6 execution plan 继续累积。当前 M6.3 原生批次在五个平台上增加已安装 Stage 5→current 升级、current→Stage 5 回滚和残留 Socket 恢复合同；普通客户端 autostart 也只有在路径已证明不存在时才启动，并以独立 Unix session 避免客户端终端退出制造残留 Socket。SHA `5b72101` 的完整双 Go 本地门禁通过，PR `29681591589` 首轮 25/25，push `29681590469` 对两条已分类时序失败各最小重跑一次后 25/25；十个 native jobs 的新步骤均通过。相邻平台批次把 `ubuntu-24.04-arm` 加入主 CI 的原生完整 lifecycle，并由 provenance 强制绑定真实 ARM64 runner 与 linux/arm64 Go identity；PLAT-003/REL-006 不因新增矩阵腿提前完成。当前兼容批次同时以无 OpenSSH extensions 的独立进程虚拟 SFTP v3 server 验证 Level 0 read path、安全 publication 拒绝和 invalid UTF-8 filename list→stat→read 原始字节链；PLAT-006 转为 `Verified`，PLAT-005/REL-006 仍保留 native vendor、final protected bytes、public channel 等明确开放边界。
