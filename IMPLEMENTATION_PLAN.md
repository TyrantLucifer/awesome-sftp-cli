...
**REL-007 increment**: Three canonical review-contract RED slices require all eight review domains, executable negative-test anchors, and explicit open production/final-review boundaries. The finding ledger now records stable coverage IDs and scoped `reviewed`/`partial` dispositions. REL-007 moves to `In Progress`, not `Verified`; production Helper/Level 2, protected final artifacts, hardware/platform gaps, and final independent security review remain open.
