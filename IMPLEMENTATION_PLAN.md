...
**Hosted timing-stability increment**: Four related RED/GREEN slices based on `c81887a8fcfacd488ff88c23ead46f34f2f972e5` require explicit Helper terminal-state joining, deterministic tail polling, and a stable selected PTY target before delete input. GREEN joins the client reader, OpenSSH process, and stderr reader through `ProcessSession.Wait`; retains the tail fixture's 80 ms limit while injecting its two poll events; and requires the Stage 2 screen to show both the filename and `cache:lru | normal` before `D`. Helper race repetition passed 200 iterations, the unchanged-limit tail fixture passed a separate 200 iterations, and real Stage 2 PTY passed 10 iterations. Focused oldstable/vet/lint/docs plus exact current Go 1.26.5 `make ci` and exact oldstable Go 1.25.12 `make check` pass. No timeout, assertion, support scope, production Helper, or Level 2 policy changed; exact-SHA Hosted evidence is pending.
