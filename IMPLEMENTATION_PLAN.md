...
**Current checkpoint**: 唯一基线仍为 `312bcccbcbd54246bbe5ff9babf4f14560449176`、tree `e0316c286ce11512cb0b92c917fa29b80f9e3305`。M6.1 已完成，M6.2 historical/restore、Job-history retention 与 Helper lifecycle 前序门禁已绿；JOB-010 为 `Verified`，REL-003 仍为 `In Progress`。确定性公共打包 commit `958dec07addbe12c0c02404af4baf90558286909` 与依赖闭集 commit `99db0e822b80d01519191e9bd13a894794c018c2` 的 push/PR Hosted 均首轮 24/24 全绿。当前安装批次的 implementation commit `0fdcb9cbcc734e9674d2455ec8123eacca722498` 已通过 exact clean 双打包与完整生命周期本地 smoke。replacement SHA `f9dd5fe09e857a4d31ad91f308b2de82692a23bf` 两侧 Hosted quality 均通过 lint、打包、checksum、man、completion、identity 与 Stage 5 启动，随后因 harness 把 persistent HOME 放在不受信 `RUNNER_TEMP` 下而按生产安全策略进入 degraded；本地不安全根复现给出 `ancestor "/tmp" is writable`，受信根 5/5 升级成功，故分类为 introduced CI fixture regression 而非产品 migration regression。fix commit `b7a740010c7e7188f9514e6676a0324aa34433a2` 改用 `/var/lib/amsftp-tests/<uid>`、增加 fail-closed workflow policy，并通过 current/oldstable focused 与完整 exact workflow smoke；replacement Hosted 待执行。项目 LICENSE、完整 NOTICE、native macOS/channel install、production signing/notary/Helper/Level 2 仍开放；12 个 Stage 6 退出条件仍全部开放，production Helper distribution 与 production Level 2 保持 **CLOSED**。
