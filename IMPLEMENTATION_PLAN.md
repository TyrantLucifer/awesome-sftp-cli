...
**Hosted timing-stability increment**: Four related RED/GREEN slices at SHA `60255dc8dc5e4732ae0fe55ac11ca860390bc81c`, tree `27fe962f3c712eae16e061a06d8460542eaaedf5`, require explicit Helper terminal-state joining, deterministic tail polling, and a stable selected PTY target before delete input. GREEN joins the client reader, OpenSSH process, and stderr reader through `ProcessSession.Wait`; retains the tail fixture's 80 ms limit while injecting its two poll events; and requires the Stage 2 screen to show both the filename and `cache:lru | normal` before `D`. Helper/tail repetition and real Stage 2 PTY pass, as do focused oldstable/vet/lint/docs and exact dual-Go full local gates. Push `29659577751` finished 21 success / 1 failure / 2 dependency-skipped after the unchanged 20 ms local-command cancellation fixture expired before process start; same-SHA PR quality passed. PR `29659579470` finished 22 success / 1 failure / 1 dependency-skipped and exposed the introduced immediately-exiting exact-stderr child's `os/exec` stdout-close handshake race; same-SHA push oldstable Intel passed. The correction restores the resident child and closes the session through the new terminal barrier before diagnostic assertion; both toolchains pass 200 repetitions and complete final local gates. No timeout, diagnostic assertion, support scope, production Helper, or Level 2 policy changed; corrected exact-SHA Hosted evidence is pending.
