...
**Current checkpoint**: 唯一基线仍为 `312bcccbcbd54246bbe5ff9babf4f14560449176`、tree `e0316c286ce11512cb0b92c917fa29b80f9e3305`。M6.1 已完成；M6.2 的 migration/package/native/preview Homebrew 独立切片已有 exact-SHA 证据，真实 public channel、项目 LICENSE 与 final protected bytes 仍开放。M6.3 foundation SHA `ae172d9bb419bc6b0b05cf3ef3467258496d463f` 建立 shared export redaction 与 deterministic bounded support-bundle core；implementation SHA `7aa6f20d1cada28064cfd4f169442ef219a9b1a8`、tree `df6450899ae9316600d38f72f426f3a4c308ce91` 又交付公开 `amsftp doctor`、help/man/三种 completion、versioned human/JSON 输出以及真实只读 system probes。stopped DB 走 immutable compiled-head validation，live sidecars 只 warn；`ssh -G -T` 有 64 KiB/stream 上限并拒绝重复安全字段/未知 host-key policy；直连 endpoint 只做 5 秒 TCP connect，proxy 配置明确不 probe。隔离 HOME smoke 证明不创建 config/state/cache/runtime。exact current Go 1.26.5 `make ci` 与 oldstable Go 1.25.12 `make check` 通过。support-bundle 的真实 source composition、CLI preview、private no-replace publication/cleanup 与完整 secret matrix 尚未完成，因此 SEC-012/OBS-009/OBS-010 仍仅为 `In Progress`。12 个 Stage 6 退出条件仍全部开放，production Helper distribution 与 production Level 2 保持 **CLOSED**。

**Latest M6.3 increment**: doctor implementation SHA `7aa6f20d1cada28064cfd4f169442ef219a9b1a8`、tree `df6450899ae9316600d38f72f426f3a4c308ce91` 完成四个 RED/GREEN 切片：immutable stopped-DB/filesystem bytes、bounded non-connecting OpenSSH config expansion/direct TCP reachability、真实 local probe composition、CLI/help/man/completion parity。安全复核追加 RED，证明重复 `ssh -G` 单值字段与未知 `StrictHostKeyChecking` 曾被接受；GREEN 后两类都 fail closed 且错误不回显 subprocess bytes。focused normal/race/vet 与 isolated-HOME real CLI smoke 通过。首轮 full current gate 在此前所有 check 通过后仅由 lint 发现 test redaction canary 的 G101 annotation，行级 justified suppression 后重启；下一轮因本批多个 fresh GOCACHE 将 `/tmp` 压到 126–130 MiB、低于未修改的 134,332,416-byte migration gate 而失败，删除明确的 disposable batch caches 后从头通过 exact current `make ci`，随后 exact oldstable `make check` 通过。没有放宽断言、timeout 或空间阈值。Hosted exact-SHA evidence 尚待 push。此前 docs SHA `20bc338a8a3d550037ad1e37762fd14104680cd0` push 24/24；PR 的 Ubuntu oldstable stderr-overflow 与 macOS ARM PTY-selection failures 都有同 SHA push companion，分类为既有 reader/selection visibility timing instability，未 rerun。support-bundle CLI/publication/cleanup 是下一批。
