...
**Current checkpoint**: 唯一基线仍为 `312bcccbcbd54246bbe5ff9babf4f14560449176`、tree `e0316c286ce11512cb0b92c917fa29b80f9e3305`。Owner-only `0.1.0-internal` preview SHA `334b1c2f001048c9857008eb602bf3cb3ecf6479` 已通过完整 push workflow `29677392555`，但 unsigned/non-redistributable 且不冒充 1.0。M6.1 已完成；M6.2 的 migration/package/native/preview Homebrew 独立切片已有 exact-SHA 证据，真实 public channel、项目 LICENSE 与 final protected bytes 仍开放。当前 REL-011/012 批次补齐 public `daemon start` 的 exact residual-Socket 恢复：只有 validated private Unix Socket 且 instance lock 明确不活跃才可进入普通 starter；status 保持无写，实际 unlink 只能由取得 matching lock 的 daemon 完成，live/uncertain/unsafe path 继续 fail closed。PLAT-009 已 `Verified`，其余 15 行仍保持 `In Progress`；macOS Kerberos、vendor SFTP、Linux arm64 native、production-Helper artifact 扫描与 final review 仍开放。12 个 Stage 6 退出条件仍全部开放，production Helper distribution 与 production Level 2 保持 **CLOSED**。
